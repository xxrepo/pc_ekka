...
uses Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms,Dialogs,
     Menus, ExtCtrls, ComCtrls, StdCtrls, Util, Grids, EKKAU, MainU, DataModuleU, MarryFont,
     FileCtrl, PrintReport, MMSystem, DB, DBGrids, ADODB, Printers, ComObj, Math, IniFiles,
     DateUtils, MPlayer, ShellApi;
...
procedure UpdateCardInfo(N:Int64; var IsLink:Boolean);
...
procedure PrintCennikList(qrL:TADOQuery; Flag:Integer; FUserID:Integer; IsUpak:Boolean; IsStiker:Boolean);
...
uses ThPrintChpU, ShowTextU, TimerZU, WebGeptralU, PiggyBankU;
     ...
     ST_ZREP:begin
              ...
              if P1=0 then
               begin
                ...
                if Iter>0 then raise EAbort.Create('Z-отчет не снят!');
               end else Exit;

              if ((EKKA.TypeEKKA=EKKA_MARRY301MTM) or (CP.NumZ=-1)) and (P1=0) then
               begin
                EKKA.fpZRep;
                if EKKA.LastError<>'' then Abort;
               end;
...
//       if Prm.AptekaID in [3,68,69,70,71,72,73,75,76] then Nds:=2;
       ...
       if EKKA.TypeEKKA in [EKKA_EXELLIO,EKKA_FP2000] then Ac:=Qr.FieldByName('Art_Code').AsInteger else Ac:=0;
...
//                  EKKA.fpServiceText(1,1,0,'- - - - - - - - - - - - - - - - - - - - ');
   ...
   Case CP.TypeOplat of
    ...
    TO_CHEK:begin
             if Cp.IsPayMaster=True then Ty:=1 else
             if (CP.B1) or (CP.TypeOplatKur=DO_BEZGOT1) then Ty:=3 else
             if (Prm.AptekaID=240) and (CP.TypeOplata240=0) then Ty:=1
                                                            else Ty:=2;
...
//   if MainF.CheckBox2.Checked then EKKA.fpCancelFiscalReceipt else
    begin
     TS:=0;
     if (CP.TypeChek=TC_PILOT) {and (CP.Fransh>0)} and (CP.SumChek-CP.SumDopl>0) then
      begin
...
procedure ShowSite(NameSite:String; P:Byte=0);
 begin
  try
   ...
   if P=1 then
    begin
     ShellExecute(0,'open',PChar('iexplore.exe'),PChar(DM.Qr.FieldByName('Value').AsString),nil,SW_SHOWNORMAL);
     Exit;
    end;
