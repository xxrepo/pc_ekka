...
Uses
  ...
  StickerForBoxU, InsulinU, DocumentsExtendedU, InsulinRepU;
     ...
     for i:=1 to 100000 do
      begin
       ...
       try
        ...
        if (FId_strah>0) or
           (Param=EK_SILENT) or
           (FTypeChek=TC_PILOT) or
           (FTypeChek=TC_INSULIN) or
           (Fid_doctor>0) or
   ...
   try
{
  if (Prm.AptekaID=232) and (CP.TypeOplat=TO_CHEK) then
    ...
    if CP.TypeChek in [TC_PILOT,TC_INSULIN] then
     begin
      DM.Qr5.Close;
  ...
  try
   ...
   for i:=1 to DM.Qr6.RecordCount do
    begin
     ...
     DM.QrEx.SQL.Add('values('''+SndRecXML.XMLBody+''',1)');
...
//                        raise Exception.Create('От сервиса получена ошибка "'+Response_OperationResult+'"!'+#13+
//                                               'Дисконт на покупку не возможен!'+#13+
//                                               'Полученый ответ: '+IntToStr(Req.Status)+' - '+Req.StatusText+#13+
//                                               'ResponseText: '+Req.ResponseText+#13);
                      end
...
//        if TSpeedButton(Sender).Down then Type_Skd:=SK_FIX10 else Type_Skd:=SK_NONE;
...
delete from skd_limit where type_skd=30 and art_code in (249244,249242,249619)
...
delete from skd_limit where type_skd=51
insert into skd_limit(art_code,type_skd,skd,fix,dtstart,dtend) values(150718,51,25,1,'2018-03-01','2018-03-31 23:59:59')
insert into skd_limit(art_code,type_skd,skd,fix,dtstart,dtend) values(150719,51,25,1,'2018-03-01','2018-03-31 23:59:59')
...
{
Описание возвращаемых значений OperationResult:
OperationResult     Description
--------------------------------------------------------------------------------
ОК                  Можно проводить продажу
pharm_not_active    Аптека (карта аптеки) не активна.
no_pharm_card       Нет такой карты аптеки
no_patient_card     Нет такой карты пациента
card_blocked        Карта пациента заблокирована
not_patient_card    Указанная карта не является картой пациента
drug_limit          Превышен лимит покупок по карте
no_today_price      Не установлена цена на препарат в личном кабинете аптеки
no_db_or_drug_link  Дистрибьютор не назначен аптеке, либо препарат не назначен дистрибьютору. Продажа не возможна.
}
