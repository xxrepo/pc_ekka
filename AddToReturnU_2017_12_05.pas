...
uses
  Forms, Controls, StdCtrls, Buttons, Classes, DB, ADODB, SysUtils;
//  , DateUtils, Dialogs,
...
var
  ...
  begin
    ...
    begin
      ...
      SQL.Add('set KOL = '+IntToStr(count_check+StrToInt(AddToReturnF.edKol.Text))+',');
      if trim(Edit1.Text)<>'' then SQL.Add('  VzDescr = '+trim(Edit1.Text)+',');
      SQL.Add('  pr_vz_post = '+ComboBox2.Items[ComboBox1.ItemIndex]+',');
      SQL.Add('  pr_vz = '''+ComboBox1.Items[ComboBox1.ItemIndex]+''',');
      SQL.Add('  CompName = host_name(),');
      SQL.Add('  ID_USER = '+IntToStr(Prm.UserID));
      SQL.Add('');
      SQL.Add('where ART_CODE = '+ReturnToProviderF.ADOQuery2.FieldByName('ART_CODE').AsString);
//      SQL.Add('where ART_CODE = '+ReturnToProviderF.ADOQuery3.FieldByName('ART_CODE').AsString);
      //SQL.SaveToFile('1234');
{
      SQL.Add('update chk_vozr ');
      SQL.Add('set KOL = :kol,');
...
begin
  ...
  begin
    ...
    begin
      SQL.Clear;
      SQL.Add('select * from APTEKA_NET.dbo.chk_vozr (nolock) where art_code = :art_code');
      Parameters.Clear;
