...
Uses OpenLib, Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls,
     ...
     CPDrv, ImageEn, ExtImage, StrUtils, Printers;
...
//  if sbMag.Down  then DM.spY_CalcSKD.Parameters.ParamValues['@ncMag']:=StrToInt(Copy(IntToStr(NumCardMag),1,3)) else
  ...
  if TypeChek in [TC_PILOT,TC_INSULIN] then
   begin
  ...
  if (Prm.AptekaID=240) and (FOrderID>0) then DM.spY_CalcSKD.Parameters.ParamValues['@OrderID']:=FOrderID
  ...
  begin
{
...

 {
   ...
   IsShowOpt:=False;
   if (TypeChek<>TC_PILOT) and (TypeChek<>TC_INSULIN) then
    begin
   ...
   if Not ( (FID_strah>0) or (TypeChek=TC_PILOT) or (TypeChek=TC_INSULIN)) then
...
//   DM.spY_FilterSklad.Locate('ART_CODE',ArtCode,[loCaseInsensitive,loPartialKey]);
   ...
   if (Prm.AptekaID<>240) and (Prm.FirmID<>14) and (Prm.FirmID<>15) and (Prm.FirmID<>16) {and (Prm.AptekaID<>322)} then
...

{
     ...
     for i:=1 to 100000 do
      begin
       if i>1 then
        begin
         if (Fid_strah>0) or (TypeChek=TC_PILOT) or (TypeChek=TC_INSULIN) or (Param=EK_SILENT) then Break;
         Ep.id_akcii:=0;
         Ep.id_prior:=0;
         if i=2 then
          begin
       ...
       try
        ...
        if (FId_strah>0) or (Param=EK_SILENT) or (FTypeChek=TC_PILOT) or (FTypeChek=TC_INSULIN) then DM.spY_EnterKol.Parameters.ParamValues['@simp']:=1
...
procedure TMainF.btnOChekClick(Sender:TObject);
 begin
  if Prm.AptekaID=240 then N3Click(N3) else
   begin
    TypeChek:=TC_MAIN;
    OpenChek;
   end;
...
//   CP.NumMediCard:=StrToInt64('272'+Copy(IntToStr(NumMediCard),4,10));
   ...
   CP.SumDopl:=0;
   if TypeChek in [TC_PILOT,TC_INSULIN] then CP.SumDopl:=FSumDopl;
  ...
  begin
   ...
   if (CP.TypeOplat in [TO_CASH,TO_CHEK,TO_CHP]) or ( (TypeChek in [TC_SERTIF,TC_STRETCH]) ) then
    ...
    if NumCardDnepr<>-1 then
     ...
     try
      if (CP.id_strah>0) or (CP.TypeChek in [TC_PILOT,TC_INSULIN]) then
       begin

        if CP.id_strah>0 then SumDopl:=CP.Fransh else
        if CP.TypeChek in [TC_PILOT,TC_INSULIN] then SumDopl:=CP.SumDopl;
   ...
   try
{
  if (Prm.AptekaID=232) and (CP.TypeOplat=TO_CHEK) then
    ...
    if CP.TypeChek=TC_PILOT then
     begin
      DM.Qr5.Close;
      DM.Qr5.SQL.Text:='exec spY_LimitDLSum '+IntToStr(Prm.UserID);
      DM.Qr5.Open;
      if DM.Qr5.FieldByName('Res').AsInteger=0 then
       begin
    ...
    if CP.TypeCardNew<>1 then
     begin
      ...
      if CP.TypeChek in [TC_PILOT,TC_INSULIN] then CP.SumOplata:=CP.SumDopl
...
//                DM.QrEx.SQL.SaveToFile('C:\JournZSums.txt');
     ...
     if (CP.id_strah>0) or (CP.TypeChek in [TC_PILOT,TC_INSULIN]) then EKKA.fpCheckCopy;

     if FIsKurDostMakeOut=True then
      begin
       EKKA.fpCashOutput(CP.SumChek,2);
       ...
       if EKKA.EmulEKKA=False then
        try
         ss:=CurrToStrF(CP.SumChek,ffFixed,2);
...
procedure TMainF.btnPayChekClick(Sender: TObject);
 begin
  if (Prm.AptekaID=240) or (ID_Doctor>0) then
   begin
    ChoosePayF:=TChoosePayF.Create(Self);
    try
    ...
    if (DM.TimerQr.FieldByName('Cnt').AsInteger>0) then
     try
      ...
      try
       ...
       if ((T<StrToTime('23:45:00')) or (T>StrToTime('00:15:00'))) and (Prm.AptekaID<>240) then
        if B then
         begin
...
//   if TWinControl(Sender).Tag=2 then
   ...
   try
    ...
    if Prm.AptekaID<>240 then IDRes_Kur:=0;
...
function TMainF.MediCardError(err:String):Boolean;
 begin
  Result:=True;
  if err='0' then raise EAbort.Create('Ошибка не определена') else
  if err='1' then raise EAbort.Create('Такой карты в базе нет') else
  if err='1a' then raise EAbort.Create('Запрашиваемая карта заблокирована') else
  if err='1b' then raise EAbort.Create('Указанная карта не активна. Скидка недоступна') else
  if err='2' then raise EAbort.Create('Обработка запроса, ожидайте 10 минут') else
  if err='3' then raise EAbort.Create('Невозможно сохранить продажу, неверный идентификатор') else
  if err='3a' then raise EAbort.Create('Невозможно сохранить продажу, нет первого запроса') else
  if err='4' then raise EAbort.Create('Невозможно сохранить продажу, запрос устарел') else
  if err='4a' then raise EAbort.Create('Невозможно сохранить продажу, запрос устарел') else
  if err='5' then raise EAbort.Create('Товар с таким кодом для Вашей сети аптек отсутствует в базе. Скидка не доступна') else
  if err='5a' then raise EAbort.Create('Товар с таким кодом отключен в базе. Скидка не доступна') else
  if err='5b' then raise EAbort.Create('Товар с таким кодом отключен в базе. Скидка не доступна') else
  if err='5c' then raise EAbort.Create('Ваша аптека и товар с таким кодом отключены в базе. Скидка не доступна') else
  if err='5d' then raise EAbort.Create('Ваша аптека отключена в базе. Скидка не доступна') else
  if err='5e' then raise EAbort.Create('Ваша аптека не найдена в базе. Скидка не доступна') else
  if err='5f' then raise EAbort.Create('Не указан договор дистрибьютора. Скидка не доступна') else
  if err='5g' then raise EAbort.Create('Доступ к сервису отключен. Скидка не доступна') else
  if err='5h' then raise EAbort.Create('Не указан код аптеки. Скидка не доступна') else
  if err='6' then raise EAbort.Create('Группа товаров с таким кодом отключен в базе. Скидка не доступна') else
  if err='6a' then raise EAbort.Create('Период действия программы для этой группы товаров истек. Скидка не доступна') else
  if err='7' then raise EAbort.Create('Указано нулевое значение количества. Скидка недоступна.') else
  if err='7a' then raise EAbort.Create('Указано дробное значение количества. Скидка недоступна.') else
  if err='7b' then raise EAbort.Create('Для мультибонусов доступно продать одну упаковку в чеке. Указанное количество больше 1. Скидка недоступна.') else
  if err='8' then raise EAbort.Create('Выбранный товар не участвует в данной программе. Скидка недоступна.') else
  if err='8a' then raise EAbort.Create('Ваша карта не обслуживается в этом городе. Скидка недоступна.') else
  if err='8b' then raise EAbort.Create('Ваша карта не обслуживается в этой аптеке. Скидка недоступна.') else
  if err='9' then raise EAbort.Create('Продажа данной группы товаров не доступно для этой карты. Скидка не доступна') else
  if err='9a' then raise EAbort.Create('Продажа данной группы товаров не доступно для этой карты. Скидка не доступна') else
  if err='9b' then raise EAbort.Create('Для указанной карты не сопоставлено значение МП и производитель. Скидка не доступна. Скидка не доступна') else
  if err='10' then raise EAbort.Create('Для указанной карты превышен лимит покупок по карте. Скидка не доступна.') else
  if err='10a' then raise EAbort.Create('Для указанной карты превышен лимит покупок по выбранной группе. Скидка не доступна.') else
  if err='11' then raise EAbort.Create('Ошибка выбора схемы скидки. Скидка не доступна.') else
  if err='11a' then raise EAbort.Create('Ошибка выбора схемы скидки. Скидка не доступна.') else
  if err='11b' then raise EAbort.Create('Ошибка выбора схемы скидки. Скидка не доступна.') else
  if err='12' then raise EAbort.Create('Срок действия плана покупок истек. Скидка не доступна.') else
  if err='12a' then raise EAbort.Create('Плана покупок не определен. Скидка не доступна.') else
  if err='12b' then raise EAbort.Create('Плана покупок не определен. Скидка не доступна.') else
  if err='12c' then raise EAbort.Create('Плана покупок не определен. Скидка не доступна.') else
  if err='12d' then raise EAbort.Create('Доступно количество упаковок к выбору только 1. Скидка не доступна.') else
  if err='12e' then raise EAbort.Create('Лимит плановых покупок исчерпан. Скидка не доступна.') else
  if err='12f' then raise EAbort.Create('Ошибка выбора дизировки. Скидка не доступна.') else
  if err='13' then raise EAbort.Create('Не указан поставщик. Скидка не доступна.') else
  if err='13a' then raise EAbort.Create('Указанный поставщик не активен или указан не верный ИД. Скидка не доступна.') else
  if (err='100') or (err='101') or (err='102') or (err='103') or (err='104') then raise EAbort.Create('Mysql запрос вернул пустой результат. Скидка не доступна') else Result:=False;
 end;
...
// DM.Qr6.SQL.Add('select a.date_Chek,b.art_code,a.cardpref,a.skd,a.numb_chek,a.cena*dbo.GetKoefUpak(b.art_code),IsNull(Sum(a.SumSkd),0) as sumskd,'+' Sum(a.kol/dbo.GetKoefUpak(b.art_code)) as kol,Sum(b.ostat/dbo.GetKoefUpak(b.art_code)) as ostat');
   ...
   DM.Qr6.Close;
   DM.Qr6.SQL.Text:='exec spY_MediCardList '+IntToStr(CP.NumbChek)+','+IntToStr(CP.Vzh);
   DM.Qr6.Open;
   ...
   for i:=1 to DM.Qr6.RecordCount do
    begin
     ...
     SndRecXML.XMLBody:='<?xml version="1.0" encoding="UTF-8"?>'+#10+
                        ...
                        ' <card_code>'+GetNumMediCard(Nc)+'</card_code>'+#10+
...
procedure TMainF.DeleteKurZakaz(IDRes:Integer);
 begin
  if (Prm.AptekaID=240) and (IDRes>0) then
   try
    DM.QrEx.Close;
    DM.QrEx.SQL.Clear;
...
delete from skd_limit where type_skd=40 and art_code in
(
206764,
206763,
206767,
...
insert into skd_limit(art_code,type_skd,skd,fix,dtstart,dtend) values(206764,40,50,1,'2017-09-01','2017-09-30 23:59:59')
insert into skd_limit(art_code,type_skd,skd,fix,dtstart,dtend) values(206763,40,50,1,'2017-09-01','2017-09-30 23:59:59')
insert into skd_limit(art_code,type_skd,skd,fix,dtstart,dtend) values(206767,40,50,1,'2017-09-01','2017-09-30 23:59:59')
insert into skd_limit(art_code,type_skd,skd,fix,dtstart,dtend) values(206768,40,50,1,'2017-09-01','2017-09-30 23:59:59')
insert into skd_limit(art_code,type_skd,skd,fix,dtstart,dtend) values(206770,40,50,1,'2017-09-01','2017-09-30 23:59:59')
insert into skd_limit(art_code,type_skd,skd,fix,dtstart,dtend) values(206771,40,50,1,'2017-09-01','2017-09-30 23:59:59')

}
{
    MessBox('При формировании окна произошла ошибка: 0'+#13+E.Message,0); //ОК без иконок
    MessBox('При формировании окна произошла ошибка: 1'+#13+E.Message,1); //ОК-Отмена без иконок
    MessBox('При формировании окна произошла ошибка: 2'+#13+E.Message,2); //Прервать/Повтор/Пропустить без иконок
    MessBox('При формировании окна произошла ошибка: 3'+#13+E.Message,3); //Да/Нет/Отмена без иконок
    MessBox('При формировании окна произошла ошибка: 4'+#13+E.Message,4); //Да/Нет без иконок
    MessBox('При формировании окна произошла ошибка: 5'+#13+E.Message,5); //Повтор/Отмена без иконок
    MessBox('При формировании окна произошла ошибка: 6'+#13+E.Message,6); //Отмена/Повторить/Продолжить без иконок

    MessBox('При формировании окна произошла ошибка: 16'+#13+E.Message,16); //ОК ошибка
    MessBox('При формировании окна произошла ошибка: 17'+#13+E.Message,17); //ОК/Отмена ошибка
    MessBox('При формировании окна произошла ошибка: 18'+#13+E.Message,18); //Прервать/Повтор/Пропустить ошибка
    MessBox('При формировании окна произошла ошибка: 19'+#13+E.Message,19); //Да/Нет/Отмена ошибка
    MessBox('При формировании окна произошла ошибка: 20'+#13+E.Message,20); //Да/Нет ошибка
    MessBox('При формировании окна произошла ошибка: 21'+#13+E.Message,21); //Повтор/Отмена ошибка
    MessBox('При формировании окна произошла ошибка: 22'+#13+E.Message,22); //Отмена/Повторить/Продолжить ошибка

    MessBox('При формировании окна произошла ошибка: 32'+#13+E.Message,32); //ОК вопрос
    MessBox('При формировании окна произошла ошибка: 33'+#13+E.Message,33); //ОК/Отмена вопрос
    MessBox('При формировании окна произошла ошибка: 34'+#13+E.Message,34); //Прервать/Повтор/Пропустить вопрос
    MessBox('При формировании окна произошла ошибка: 35'+#13+E.Message,35); //Да/Нет/Отмена вопрос
    MessBox('При формировании окна произошла ошибка: 36'+#13+E.Message,36); //Да/Нет вопрос
    MessBox('При формировании окна произошла ошибка: 37'+#13+E.Message,37); //Повтор/Отмена вопрос
    MessBox('При формировании окна произошла ошибка: 38'+#13+E.Message,38); //Отмена/Пропустить/Продолжить вопрос

    MessBox('При формировании окна произошла ошибка: 48'+#13+E.Message,48); //ОК восклицание
    MessBox('При формировании окна произошла ошибка: 49'+#13+E.Message,49); //ОК/Отмена восклицание
    MessBox('При формировании окна произошла ошибка: 50'+#13+E.Message,50); //Прервать/Повтор/Пропустить восклицание
    MessBox('При формировании окна произошла ошибка: 51'+#13+E.Message,51); //Да/Нет/Отмена восклицание
    MessBox('При формировании окна произошла ошибка: 52'+#13+E.Message,52); //Да/Нет восклицание
    MessBox('При формировании окна произошла ошибка: 53'+#13+E.Message,53); //Повтор/Отмена восклицание
    MessBox('При формировании окна произошла ошибка: 54'+#13+E.Message,54); //Отмена/Повторить/Продолжить восклицание

    MessBox('При формировании окна произошла ошибка: 64'+#13+E.Message,64); //ОК информация
    MessBox('При формировании окна произошла ошибка: 65'+#13+E.Message,65); //ОК/Отмена информация
    MessBox('При формировании окна произошла ошибка: 66'+#13+E.Message,66); //Прервать/Повтор/Пропустить информация
    MessBox('При формировании окна произошла ошибка: 67'+#13+E.Message,67); //Да/Нет/Отмена информация
    MessBox('При формировании окна произошла ошибка: 68'+#13+E.Message,68); //Да/Нет информация
    MessBox('При формировании окна произошла ошибка: 69'+#13+E.Message,69); //Повтор/Отмена информация
    MessBox('При формировании окна произошла ошибка: 70'+#13+E.Message,70); //Отмена/Повторить/Продолжить информация
}
...
До придбаних Вами товарів
рекомендуємо зі знижкою
Пробиотик+Пребиотик 2 блистера
по 10шт
по промокоду 00473-86947-76613
на сайті apteka911.com.ua

Отримуй знижку 5% при реєстрації на сайті apteka911.com.ua
}
...
delete from skd_limit where type_skd=30 and art_code in (247308,247309,247324,247093)
insert into skd_limit(art_code,type_skd,skd,fix,dtstart,dtend) values(247093,30,0,1,'2018-02-01','2018-03-31 23:59:59')
insert into skd_limit(art_code,type_skd,skd,fix,dtstart,dtend) values(247308,30,99.99,1,'2018-02-01','2018-03-31 23:59:59')
insert into skd_limit(art_code,type_skd,skd,fix,dtstart,dtend) values(247309,30,99.99,1,'2018-02-01','2018-03-31 23:59:59')
...
insert into skd_limit(art_code,type_skd,skd,fix,dtstart,dtend) values( ,31,99.99,1,'2018-02-13','2018-02-28 23:59:59')


delete from skd_limit where type_skd=31

insert into skd_limit(art_code,type_skd,skd,fix,dtstart,dtend) values(129756,31,0,1,'2018-02-13','2018-02-28 23:59:59')
insert into skd_limit(art_code,type_skd,skd,fix,dtstart,dtend) values(91816 ,31,0,1,'2018-02-13','2018-02-28 23:59:59')
insert into skd_limit(art_code,type_skd,skd,fix,dtstart,dtend) values(91817 ,31,0,1,'2018-02-13','2018-02-28 23:59:59')
insert into skd_limit(art_code,type_skd,skd,fix,dtstart,dtend) values(91818 ,31,0,1,'2018-02-13','2018-02-28 23:59:59')
insert into skd_limit(art_code,type_skd,skd,fix,dtstart,dtend) values(18660 ,31,99.99,1,'2018-02-13','2018-02-28 23:59:59')
insert into skd_limit(art_code,type_skd,skd,fix,dtstart,dtend) values(86093 ,31,99.99,1,'2018-02-13','2018-02-28 23:59:59')
insert into skd_limit(art_code,type_skd,skd,fix,dtstart,dtend) values(18655 ,31,99.99,1,'2018-02-13','2018-02-28 23:59:59')
insert into skd_limit(art_code,type_skd,skd,fix,dtstart,dtend) values(18659 ,31,99.99,1,'2018-02-13','2018-02-28 23:59:59')
