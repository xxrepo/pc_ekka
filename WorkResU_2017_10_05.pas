...
var V:Boolean;
 begin
  ...
  if DM.qrRes.FieldByName('Priznak').AsInteger in [3] then // Если бронь со склада или с аптеки на аптеку, то отмену брони отменяем
   begin
    bbClearRes.Visible:=False;
    Exit;
   end;
...
procedure TWorkResF.BitBtn4Click(Sender: TObject);
 begin
  ...
  PrintRep.AddText('Заказ №'+DM.qrRes.FieldByName('NumZakaz').AsString+#10);
  PrintRep.AddText('ФИО: '+DM.qrRes.FieldByName('FIO').AsString+#10);
  PrintRep.AddText('Дата бронирования: '+DM.qrRes.FieldByName('Date_Res').AsString+#10#10);
