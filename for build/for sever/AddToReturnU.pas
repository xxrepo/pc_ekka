...
var
  ...
  begin
    ...
    begin
      ...
      SQL.Add('  s.KOD_NAME, s.ART_CODE, s.NAMES, s.CENA, s.F_NDS, s.ART_NAME,');
      ...
      begin
        ...
        if not ReturnToProviderF.aTmp.FieldByName('NAMES').IsNull then check[i].NAMES:=ReturnToProviderF.aTmp.FieldByName('NAMES').AsString else check[i].NAMES:='_empty_';
        ...
        if not ReturnToProviderF.aTmp.FieldByName('ART_NAME').IsNull then check[i].ART_NAME:=ReturnToProviderF.aTmp.FieldByName('ART_NAME').AsString else check[i].ART_NAME:='_empty_';
        check[i].DATEINS:=Date();
        check[i].PR1:=-1;
        check[i].CompName:=CompName;
        if not ReturnToProviderF.aTmp.FieldByName('SKLAD').IsNull then check[i].SKLAD:=ReturnToProviderF.aTmp.FieldByName('SKLAD').AsString else check[i].SKLAD:='_empty_';
        ...
        if not ReturnToProviderF.aTmp.FieldByName('nn_prih').IsNull then check[i].nn_prih:=ReturnToProviderF.aTmp.FieldByName('nn_prih').AsString else check[i].nn_prih:='_empty_';
        ...
        if trim(Edit1.Text)<>'' then check[i].VzDescr:=Edit1.Text else check[i].VzDescr:='_empty_';
        ...
        if not ReturnToProviderF.aTmp.FieldByName('NumSeriya').IsNull then check[i].NumSeriya:=ReturnToProviderF.aTmp.FieldByName('NumSeriya').AsString else check[i].NumSeriya:='_empty_';
...
//          check[i].KOL:=check[i].KOL-kol_return;
          fl_break:=true;
        end
        else
        ...
        SQL.Add('  pr_vz, id_part8, NumSeriya, SrokGodn ');
        SQL.Add(') values (');
        ...
        SQL.Add('  convert(uniqueidentifier,@id_part8), @NumSeriya, convert(datetime,@SrokGodn)'); //id_part8 //NumSeriya //SrokGodn
        SQL.Add(')');
        Parameters.Clear;
...
begin
  ...
  begin
    ...
    begin
      SQL.Clear;
      SQL.Add('select * from ##Check (nolock) where art_code = :art_code');
      Parameters.Clear;
