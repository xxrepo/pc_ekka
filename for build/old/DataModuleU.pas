...
uses Dialogs, Forms, SysUtils, Classes, DB, ADODB, MainU, Math, Variants;
...
type
  ...
  public
    ...
    procedure InsertArhChek(CP:TChekPos);
...
uses OtkazU, ShareU, PeresortU, WorkResU, AkciiU, ProvizInfoU, CountersU, ViewVzResU, DocumentsU;
...
procedure TDM.InsertArhChek(CP:TChekPos);
 begin
  try
...
var i:Integer;
 ...
 begin
  try
   ...
   if CP.TypeChek=TC_KURER then DM.spY_WriteChek.Parameters.ParamValues['@ptypeopl']:=CP.TypeOplat+2
    else
   begin
    if (Prm.AptekaID=240) or (CP.IsPayMaster=True) then
   ...
   if CP.IsTerminal then
    begin
     ...
     DM.spY_WriteChek.Parameters.ParamValues['@Merchant' ]:=CP.Merchant;
              ...
              DM.spY_WriteChek.Parameters.ParamValues['@InvoiceNum']:=Null;
              DM.spY_WriteChek.Parameters.ParamValues['@Merchant']:=Null;
             end;
