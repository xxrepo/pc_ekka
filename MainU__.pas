...
Uses OpenLib, Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms, Dialogs,
     ...
     PrintReport, USBEject, IdMessage, IdBaseComponent, IdComponent,
     ...
     InvokeRegistry, SOAPHTTPClient, SOAPHTTPTrans, OpenOffice,
     Types, Printers, svButtons, Math, IdTCPServer, Registry, MPlayer, SHDocVw, CPDrv,
     RpDefine, RpRender, RpRenderCanvas, RpRenderPreview, ImageEn;
...
Type TFake=class(TCustomGrid);
     ...
     TFileOpt=Record               // Структура ini-файла
               ...
               ConStrMess:String;     // Строка подключения для сообщений в офис на DB
               ConStrMessS2014:String;// Строка подключения для сообщений в офис на S2014
     ...
     TPrParams=Record               // Параметры программы
                ...
                KoefZakaz:Integer;
                ServerDB:String;      // Имя главного офисного SQL сервера
                ...
                ReklamaOff:String;
                IsSoc:Boolean;
                FilterSkladFast:Boolean;  // Режим работы программы, когда наименования свернуты по "сделано из" и используется быстрая фильтрация и сортировка
               end;
     ...
     TChekPos=Record   // Параметры позиции чека
               ...
               TypeChek:Integer;
               Sums:Array[0..11] of Real; // Текуще состояние кассового аппарата
               VzhS:String;
               NumZ:Integer;
               Fn:Int64;
               RealizDay:Currency;
               ...
               DateK:TDateTime;    // Дата время кассовго аппарата
    ...
    procedure dbgSkladKeyUp(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    ...
    procedure bbWhereIsClick(Sender: TObject);
    ...
    procedure BitBtn9Click(Sender: TObject);
     ...
     private
       ...
       procedure GetNumCardDialog(var NumCard:Int64; var TypeCard:Integer; var TC:Byte; var AccumSum,SpSum:Currency; CheckOnly:Boolean=False);
       ...
       function  PayChek(TypeOplat:Integer; B1:Byte=0):Boolean;
       ...
       function  GetKolFromDialogAdv(Names:String; DefKol,Art_Code:Integer):Integer;
     ...
     public
       ...
       procedure ShowTovInfo(Art_Code:Integer; Names:String; Upak:Integer; P:Byte=0);
       ...
       function  MessBox(Mes:String; Btn:Integer; F:TFont; var Res:String; URes:Boolean=False; Caption:String=''; bBtn:TWinControl=nil):Integer; overload;
       ...
       function  GetKolFromDialog(Nm:String; DefKol:Integer; var Art_Code:Integer; P:Byte; Koef:Integer=0; Upak:Integer=0; Cap:String=''):Integer;
       ...
       function  CheckMakeFromExists(ds:TADOStoredProc):Boolean;
...
//      CR_STR241='241'; // Скретч карты (Сертификаты)
      ...
      { --- Признpa.SP8991=pr.iddocаки формирования пересорта ---}
...
var S:String;
 ...
 begin
  ...
  try
   DM.Qr.Close;
   DM.Qr.SQL.Clear;
   DM.Qr.SQL.Add('delete from Inform..JReadMess where id_mess in (select id from Inform..JMess (nolock) where DateM<getdate()-30 and IsNull(IsZp,0)=0)');
   DM.Qr.SQL.Add('delete from Inform..JMess where DateM<getdate()-10 and IsNull(IsZp,0)=0 ');
   DM.Qr.ExecSQL;
  except
  end;
   ...
   DM.Qr.SQL.Add('insert into RegPredst(id_p,dt,flag) values(@id_p,@dt,@flag) ');
{
   ...
   DM.Qr.SQL.SaveToFile('C:\RegPr.txt');
   DM.Qr.ExecSQL;
  except
  end;
  ...
  try
   ...
   DM.TimerQr1.SQL.Add('update DhRes set Closed=1,id_close=0,date_close=getdate() where IsNull(closed,0)=0 and date_res<(case when TypeZakaz=3 then getdate()-3 else getdate()-1 end)');
   if (Prm.AptekaID=218) then
    begin
     DM.TimerQr1.SQL.Add('if getdate()<''2015-10-28'' ');
     DM.TimerQr1.SQL.Add(' update DhRes set Closed=0,id_close=null,date_close=null where idres=6688 ');
    end;
...
var Res,i,Tmp:Integer;
 ...
 Begin
  if Prm.TerminalNew then begin Result:=True; Exit; end;
  Result:=False;
...
procedure TMainF.InitFields;
 begin
  ...
  N72.Visible:=Prm.FirmID in [14,15,16,17];
  ...
  try
   DM.Qr.Close;
   DM.Qr.SQl.Text:='select * from Spr_Const (nolock) where Descr=''SkdDop''';
   DM.Qr.Open;
...

{
  ...
  if Prm.SkdFirst=False then
   try
    ...
    DM.QrEx.Close;
    DM.QrEx.SQL.Text:='select Min(date_nakl) as dn from JMoves (nolock)';
    DM.QrEx.Open;
...
//  sbPens.Visible:=Prm.AptekaID in [145..147];
  ...
  try
   DM.Qr7.Close;
   DM.Qr7.SQL.Text:='select Value from Spr_Const (nolock) where Descr=''Scaner'+IntToStr(Opt.KassaID)+''' ';
   DM.Qr7.Open;
   if DM.Qr7.IsEmpty=False then Abort;
   if SetScanerIni then
    begin
     DM.Qr7.Close;
     DM.Qr7.SQL.Text:='insert into Spr_Const(Descr,Value) values(''Scaner'+IntToStr(Opt.KassaID)+''',''1'') ';
     DM.Qr7.ExecSQL;
    end;
  except
  end;
   ...
   begin
    pnSklad.Height:=pnSkd.Height;
             ...
             dbgDown.Visible:=Not DM.qrDown.IsEmpty;
            {
             ...
             dbgSop.Visible:=Not DM.qrSop.IsEmpty;
...
//  T:=Time;
 ...
 { if DBText1.Caption='' then
  ...
  DBText1.Hint:=DBText1.Caption;
  DBText2.Visible:=DBText1.Caption<>'';
  Shape4.Visible:=DBText1.Caption<>'';
...
//  MainF.miNoTov.Enabled:=spY_FilterSklad.FieldByName('Ostat').AsInteger=0;
    ...
    if DM.spY_FilterSklad.FieldByName('InPrice').AsInteger=1 then lbPrice.Caption:='Есть у поставщиков'
                                                             else lbPrice.Caption:='Дефектура';
...
//  miReports.Visible:=Prm.AptekaID<>63;
  ...
  if (Prm.AptekaSklad) and (Prm.Dobraia=False) then
   begin
    lbCenaUpSkd.Caption:='Цена за упаковку от 3-x упаковок';
...
//  Button2.Visible:=Design;
  ...
  ShowStatus('Касса '+IntToStr(Opt.KassaID),1);
...
var CP:TChekPos;
 ...
 //   DM.QrEx.SQL.SaveToFile('C:\ghhhh.txt');
    DM.QrEx.Open;
...
{    DM.QrEx.Close;
    ...
    if (B=1) {and (B1=False)} then DoneCheck(CP,1)
                              else begin
                                    DoneCheck(CP,0);
                                    Result:=-1;
                                   end;
...
var B:Integer;
 begin
  ...
  try
   DM.QrEx.Close;
   DM.QrEx.SQL.Text:='exec spY_ClearCheks '+IntToStr(Prm.UserID);
   DM.QrEx.ExecSQL;
...
var Com,S:String;
...

 {
  ...
  {if EKKA.EmulEKKA then} ShowIndic(CenterStr(FormatDateTime('hh-nn',Now),10));
  ...
  if Prm.AptekaID<>240 then EKKA.FirmNameUA:=Prm.FirmNameUA
                       else EKKA.FirmNameUA:='ФЛП Симонян Х.С.';
  ...
  EKKA.fpSendCommand(Com);
{
  ...
  try
   if EKKA.fpGetLimitStatus then
    begin
     if StrToInt(EKKA.RD_Item[2])<100 then
      if EKKA.fpServPassw('2222222222') then
       begin
...
function TMainF.MessBox(Mes:String; Btn:Integer; F:TFont; var Res:String;URes:Boolean=False; Caption:String='';  bBtn:TwinControl=nil):Integer;
 begin
...
var chX,dx,i:Integer;
 begin
  try
   ...
   pnse.Left:=pns14.Left+pns14.Width;

  {
   ...
   DBText1.Left:=Shape4.Left+1;
   DBText1.Width:=Shape4.Width-2;
...
function TMainF.CheckSmena:Boolean;
 begin
  try
   if {Design} ChekOpened then begin Result:=True; Exit; end;
...
//     FIsSkd7:=(Prm.AptekaID=235) and (MessBox('У покупателя есть флаер на скидку 7% ?',52)=ID_YES);
...
var NC:Int64;
 ...
 begin
  ...
  try
   GetNumCardDialog(NC,TC,tcN,aSum,sSum,True);
   if NC<=0 then
    begin
  ...
  if IsPermit([R_SUPER],P_ALERT) then
   begin
    ...
    try
     ...
     lbAlertUnLoadBase.Caption:='Идет обновление данных о сотрудниках. Пожалуйста подождите!';
     ...
     try
      ...
      QrToOff.CommandTimeout:=60;
      QrToOff.Close;
      QrToOff.SQL.Text:='select ip from ekka_net..apteks (nolock) where id_apteka='+IntToStr(Prm.AptekaID);
      QrToOff.Open;
      IP:=QrToOff.FieldByName('IP').AsString;

      QrToOff.SQL.Add('insert into ['+IP+'].inform.dbo.l_employee_dplace(id,id_dept,id_employee,id_employee_position,id_payment_mode,date_begin,date_end) ');
      QrToOff.SQL.Add('select id,id_dept,id_employee,id_employee_position,id_payment_mode,date_begin,date_end from workdb..l_employee_dplace (nolock) where id not in ');
      QrToOff.SQL.Add('(select id from ['+IP+'].inform.dbo.l_employee_dplace (nolock)) and id_dept in (select id from workdb..d_dept (nolock) where id_apteka='+IntToStr(Prm.AptekaID)+') ');
      QrToOff.ExecSQL;
...
var PercSKD:Real;
 begin
  ...
  if (FIsSkd7) and (FIDRes=-1) then DM.spY_CalcSKD.Parameters.ParamValues['@pskd7']:=10
                               else DM.spY_CalcSKD.Parameters.ParamValues['@pskd7']:=0;
...
//  if sbMag.Down  then DM.spY_CalcSKD.Parameters.ParamValues['@ncMag']:=StrToInt(Copy(IntToStr(NumCardMag),1,3)) else
  if sbMag.Down  then DM.spY_CalcSKD.Parameters.ParamValues['@ncMag']:=NumCardMag
   else
...
//   if Not (((FEnterKol=True) and (FSkdCalced=False)) or (FEnterKol=False)) then Exit;
 ...
 Begin
  try
   ...
   CalcSkd;
   if (Prm.FirmID<>13) and ((SumChek>=Prm.SumSkd) or (IsCanRegMomBabyCard)) and (FIsOneRegCard) and (BlockingChek=False) and (Prm.AptekaSklad=False) and ((FIDRes=-1) or ((FIDRes<>-1) and (FTypeRes=2)) ) then
    begin
     FIsOneRegCard:=False;
     try
      sbCard.Tag:=1;
...
//   DM.spY_FilterSklad.Parameters.ParamValues['@ptov']:=GroupIndex;
   ...
   if Prm.FilterSkladFast then DM.spY_FilterSklad.ProcedureName:='spY_FilterSkladFast';
   ...
   DM.spY_FilterSklad.Parameters.ParamValues['@nmFilter']:=edCntxSearch.Text;
   ...
   end;
{
 ...
 //  FormResize(Self);
   ...
   ShowDatePrih;
   if (dbgSklad.Visible) and (P=0) then dbgSklad.SetFocus;
   ...
   if Length(EAN13)>13 then Abort;
{
   ...
   DM.QrFnd.Close;
   Case P of
    SC_SKLAD:sq:='select art_code from sprtov (nolock) where ostat>0 group by art_code';
    SC_FIND:sq:='select art_code from sprtov (nolock) where ostat>'+IntToStr(Kol-1);
    SC_CHEK:sq:='select b.art_code from sprtov a (nolock), chek b (nolock) where a.kod_name=b.kod_name';
    ...
    SC_MOVES:sq:='select a.art_code from sprtov a (nolock), rTovar b (nolock) where a.kod_name=b.kod_name';
   end;
...
//     SC_MOVES:FindInMoves;
   ...
   if Length(EAN13)=13 then
    begin
     if (AC=-1) and (P=SC_CHEK) then MessBox('Позиции с штрих-кодом '+EAN13+' в чеке нет!') else
  ...
  try
   ...
   if P=1 then
    begin
     ...
     DM.qrArtAdd.SQL.Add('where s.art_code=t.art_code and t.names like ''Пакет%'' and s.ostat>0 ');
...
//   DM.qrArtAdd.SQL.SaveToFile('C:\456.sql');
   ...
   try
    ...
    if (DM.qrArtAdd.RecordCount>1)  or (P in [0,1]) then
     begin
      Application.ProcessMessages;
     ...
     begin
      ...
      Abort;
              ...
              { DM.Qr8.Close;
               ...
               Result:=StrToFloat(GetValueFromXML('ResultDiscountAmount',sR1.DataString));
              end;
{
...
function TMainF.GetKolFromDialog(Nm:String; DefKol:Integer; var Art_Code:Integer; P:Byte; Koef:Integer=0; Upak:Integer=0; Cap:String=''):Integer;
var Kol,K:Integer;
 begin
  try
   Result:=-1;
...
function TMainF.GetKolFromDialogAdv(Names:String; DefKol,Art_Code:Integer):Integer;
 begin
  try
   Result:=-1;
   ...
   try
    ...
    EnterKolNewF.lbArt.Caption:=IntToStr(Art_Code);
    EnterKolNewF.lbArt.Tag:=Art_Code;
    EnterKolNewF.edKol.Text:=IntToStr(DefKol);

    if DM.qrSop.IsEmpty=False then
     begin
...
function TMainF.CheckMakeFromExists(ds:TADOStoredProc):Boolean;
var AC:Integer;
    Res,Nm:String;
    Srok:TDateTime;
     ...
     if ds.Locate('art_code',DM.Qr1.FieldByName('ART_CODE').AsInteger,[]) then
      begin
       if MessBox('Для позиции: '#10+nm+#10'есть сроковая или без упаковки позиция: '#10+
                   ...
                   52,GetFont('MS Sans Serif',12,clBlue,[fsBold]),Res)<>ID_YES
       then Abort
       else Result:=True;
      end else Abort;
   except
...
var Ostat,CKol,{AC,}P,KolD,i,Upak,Koef,Koef1,Kol,KolAdd,ArtAdd,Art_Code,Art_CodeSafe,Old_Art,F_NDS,Type_Map:Integer;
    Srok,T:TDateTime;
  ...
  begin
{
   ...
   if DM.Qr1.IsEmpty=False then
    MessBox('ДАННЫЙ ПРЕПАРАТ ОТПУСКАЕТСЯ СТРОГО ПО РЕЦЕПТУ'#10+
            'Напоминаем Вам что, согласно "Приказа МОЗ Украины от 19 июля 2015г. № 360" '+
            ...
            'На рецептах, при отпуске по ним лекарственных средств, проставляется штамп "Отпущено" и возвращаем больному. '+
            'Отпуск лекарственных средств по рецепту со штампом "Отпушено" запрещен. '+
            'Уважаемые сотрудники аптек при возможности при отпуске лекарственных средств по рецепту не отдавать рецепт покупателю, а оставить его в аптеке и с первой машиной передать в офис. ',
 ...
 var S,ss:String;
  ...
  begin
   FIODoctor:='';
   if Not ((AC=89722) or (AC=55679) or (AC=14290) or (AC=94997)) then begin Result:=True; Exit; end;
   Result:=False;
   Repeat
 ...
 Begin
  try
   ...
   if (TypeChek=TC_PILOT) and (FFlagPilot=False) then
    begin
     MessBox('В режиме "Пилотный проект" нельзя добавлять перпараты вручную!');
     Exit;
    end;
   if Prm.UserRole=R_DISP then
    begin
   ...
   if (Copy(IntToStr(NumCardMag),1,3)=CR_PROH) and (FFlagProh=False) then
    begin
     MessBox('В этом режиме работы программы добавлять в чек позиции нельзя!');
     Exit;
    end;
 ...
 {  if DM.qrChek.IsEmpty=False then
    if (Opt.EKKAAsPrinter=True) and (DM.qrChek.RecordCount>10) then
     begin
      MessBox('На этой кассе нельзя добавлять в чек более 10 позиций в один чек. Более 10 позиций необходимо пробивать несколькими чеками!');
      Exit;
     end;
  }
   if (DM.qrChek.IsEmpty) and (FIDRes=-1) then
    FIsSkd7:=(Prm.AptekaID=235) and (MessBox('У покупателя есть флаер на скидку 7% ?',52)=ID_YES);

   if Prm.OptCena=False then
    begin
     if (FIsSkd7=False) and (Prm.AptekaSklad) and (Prm.Dobraia=False) and (RoundTo(DM.spY_FilterSklad.FieldByName('OstatUp').AsFloat,-2)>=3) and
        (RoundTo(DM.spY_FilterSklad.FieldByName('Skd').AsFloat,-2)<RoundTo(DM.spY_FilterSklad.FieldByName('CenaUp').AsFloat,-2)) then

      MainF.MessBox('"У нас при покупке от 3 упаковок одного товара действует оптовая цена, что очень выгодно.'#10+
                    'Оптовая цена '+CurrToStrF(DM.spY_FilterSklad.FieldByName('Skd').AsFloat,ffFixed,2)+' грн. '#10+
                    'Розничная цена '+CurrToStrF(DM.spY_FilterSklad.FieldByName('CenaUp').AsFloat,ffFixed,2)+' грн. "'
                    ,48,GetFont('Times New Roman',22,$00006600,[fsBold]),Res,False,'Озвучте фразу:');
    end;
   ...
   IsAddSrok:=CheckMakeFromExists(DM.spY_FilterSklad);
...
{   if (Prm.AptekaID in [95,98,117,125,119,94,61,93,63,176]) and (TypeChek<>TC_PILOT) then
    try

     DM.Qr1.Close;
     DM.Qr1.SQL.Text:='select top 1 art_code from SprPilot (nolock) where art_code='+IntToStr(Art_Code);
     DM.Qr1.Open;

     if DM.Qr1.IsEmpty=False then
      ShowQ('Данный препарат учавствует в программе "Пилотный проект". Если у покупателя есть рецепт, вы можете отменить чек и с помощью меню "Пилотный проект" оформить продажу по спеццене!',0,True);
    except
    end;
}
   DM.Qr.Close;
...
T:=Time;
     ...
     if FIsChSeria then
      begin
       ...
       if Kol>DM.spY_FilterSklad.FieldByName('OSTAT').AsInteger then Kol:=DM.spY_FilterSklad.FieldByName('OSTAT').AsInteger;
                 ...
                 if CKol<=0 then
                  begin
                   if Prm.AdvEnterKol then Kol:=GetKolFromDialogAdv(DM.spY_FilterSklad.FieldByName('Names').AsString,Kol,Art_Code)
                                      else Kol:=GetKolFromDialog(DM.spY_FilterSklad.FieldByName('Names').AsString,Kol,Art_Code,KolD,Koef,DM.Qr1.FieldByName('Upak').AsInteger);
                  end else Kol:=CKol;
                end;
     ...
     if (Kol<DM.spY_FilterSklad.FieldByName('Upak').AsInteger) and
        (DM.spY_FilterSklad.FieldByName('IsAnnot').AsInteger=1) then
      begin
       if MessBox('Предложите клиенту распечатать инструкцию: '#10#10+
                  ...
                  ,
                  52,GetFont('MS Sans Serif',12,clBlue,[fsBold]),Res)=ID_YES then
     ...
     for i:=1 to 100000 do
      begin
       ...
       try
        ...
        Case i of
         1:begin
            if IsAddSrok then DM.spY_EnterKol.Parameters.ParamValues['@pscan']:=0
             else
...
var T:TDateTime;
 ...
 begin
  ...
  try
   ...
   try // Интернет-заказы
    DM.TimerQr.Close;
    DM.TimerQr.SQL.Text:='select Count(*) as Cnt from DhRes where Priznak=1 and IsNull(Closed,0)=0';
 //   DM.TimerQr.SQL.Text:='select Cnt from gh';
    ...
    if DM.TimerQr.FieldByName('Cnt').AsInteger>0 then
     try
      ...
      try
       ...
       if CntZakaz<>DM.TimerQr.FieldByName('Cnt').AsInteger then
        begin
         B:=True;
       ...
       pnBarDop.Tabs.Items[0].Btn.Hint:='Активных интернет заказов: '+DM.TimerQr.FieldByName('Cnt').AsString;

       PrepareIm(imIZak,DM.TimerQr.FieldByName('Cnt').AsInteger,Bm);

       pnBarDop.Tabs.Items[0].Btn.Glyph.Assign(Bm);
       pnBarDop.Tabs.Items[0].Btn.Visible:=True;

       if (B) and (CntZakaz<DM.TimerQr.FieldByName('Cnt').AsInteger) then
   ...
   try // Сообщения
    ...
    DM.TimerQr.SQL.Add('select Count(*) as Cnt from Inform..JMess where (IsNull(id_dest,-1)=-1 or id_dest='+IntToStr(Prm.ID_Gamma)+') and IsQueue=1 and id not in ');
...
//    DM.TimerQr.SQL.SaveToFile('C:\Mess.txt');
    DM.TimerQr.Open;
    pnBarDop.Tabs.Items[1].Btn.Tag:=DM.TimerQr.FieldByName('Cnt').AsInteger;

    if pnBarDop.Tabs.Items[1].Btn.Tag>0 then
     try
      Bm:=TBitMap.Create;
      try

       pnBarDop.Tabs.Items[1].Btn.ShowHint:=True;
       pnBarDop.Tabs.Items[1].Btn.Hint:='Сообщений: '+IntToStr(pnBarDop.Tabs.Items[1].Btn.Tag);

       PrepareIm(imHist,pnBarDop.Tabs.Items[1].Btn.Tag,Bm);
     ...
     end else pnBarDop.Tabs.Items[1].Btn.Glyph.Assign(imHist.Picture);
   ...
   if  ((T>StrToTime(Prm.ReklamaOn)) and (T<StrToTime(Prm.ReklamaOn)+StrToTime('00:00:08'))) then
    begin
     MessBox('Не ЗАБУДЬТЕ включить рекламу и проверить светиться реклама или нет! ');
    end;
   ...
   // Выгрузка заархивированной базы
{
   try
    DM.TimerQr.Close;
    DM.TimerQr.SQL.Text:='select * from spr_const (nolock) where descr=''ArhBase''';
    DM.TimerQr.Open;
...
procedure TMainF.GetNumCardDialog(var NumCard:Int64; var TypeCard:Integer; var TC:Byte; var AccumSum,SpSum:Currency; CheckOnly:Boolean=False);
 begin
  Case TypeChek of
...
//     if (Prm.AptekaID=63) or (FSumPChek>0) then
     ...
     if NumCard<=0 then
      begin
       GetNumCardDialog(NC,TC,tcN,aSum,sSum);
       EmptyKeyQueue;
       lbSearch.Caption:='';
       NumCard:=NC;
       TypeCard:=TC;
       ...
       if (TypeCard=1) then
        begin
         if (Prm.AptekaID in [28,166,190]) and (MessBox('Есть ли у клиента сертификат супермаркета "Fozzy" на скидку 10%?'+#10#10+
     ...
     if NumCard<=0 then
      begin
         if (Prm.AptekaID in [28,166,190]) and (MessBox('Есть ли у клиента флаер на скидку 10%?'+#10#10+
                                                        'Если ли есть, нажмите да и клиент получит скидку до 10%.',52)=ID_YES) then
          FIsSkd7:=True;
...
//                DM.Qr.SQL.Add('       IsNull((select convert(int,Sum(SumMonth)/case IsNull(Max(Old),0) when 0 then 5000 else 200 end ) from Cards where NumCard='+IntToStr(NumCard)+'),0) as SkdMn,  ');
                ...
                if Bonus<=0 then
                 begin
                  if IsFozzy then FPercentSkd:=13+GetSkdAdd;
...
var Num:String;
 begin
  ...
  if TSpeedButton(Sender).Down=True then
   if Not EnterStrValue(Num,'Введите пять последних символов с карточки на гептрал!') then TSpeedButton(Sender).Down:=False
   else begin
         FNumGeptralCard:=Num;
...
procedure TMainF.sbCardClick(Sender:TObject);
 begin
  ...
  if (Prm.FirmID=13) or (Prm.IsSoc and (FTypeChek=TC_STRETCH) ) then begin TSpeedButton(Sender).Down:=False; Exit; end;
...
// Оплата чека
function TMainF.PayChek(TypeOplat:Integer; B1:Byte=0):Boolean;
var CP:TChekPos;
    SumCard:Currency;
...
//     PlayFraz(12);
     ...
     MainF.MessBox(S1,48,GetFont('Times New Roman',22,$00006600,[fsBold]),Res,False,'Озвучте фразу:');
    except
    end;
   FTypeCardNew:=0;
   FIsTerminal:=False;
...
//   if CheckNewVersion then Exit;
 ...
 var Res,Mes:String;
  ...
  begin
   ...
   //MainF.MessBox('- "Сумма вашей покупки '+CurrToStrF(CP.SumChek,ffFixed,2)+' грн." ',48,GetFont('Times New Roman',22,$00006600,[fsBold]),Res,False,'Озвучте фразу:');

   if Not CheckSertif then raise EAbort.Create('');

   if (TypeCardNew<>1) and (Not Opt.ShowMCash) then Exit;
   if (TypeCardNew<>1) and (CP.TypeOplat=TO_CHEK) then Exit;
   B:=True;
       ...
       begin
        ...
        if CP.TypeOplat=TO_CHEK then CalcSkdOnly(0,0,False,1);
                 ...
                 if CP.SumSpis>0 then
                  MainF.MessBox('- "Сумма вашей покупки с учетом использованых накоплений '+CurrToStrF(CP.SumChek,ffFixed,2)+' грн." ',48,GetFont('Times New Roman',22,$00006600,[fsBold]),Res,False,'Озвучте фразу:')
                 else
                  if Prm.AptekaSklad=False then
                   begin
                    //PlayFraz(5,CP.SumChek);
                    MessBox('- "Сумма вашей покупки '+CurrToStrF(CP.SumChek,ffFixed,2)+' грн." ',48,GetFont('Times New Roman',22,$00006600,[fsBold]),Res,False,'Озвучте фразу:');
                   end;
 ...
 var P,Res,Mes:String;
  ...
  begin
   try
    ...
    if (Prm.IsScanAll) and (CheckScan(P)=True) and (Prm.UserRole<>R_ADMIN) then
     begin
...
{    if (IsCHP) and (TypeOplat=TO_CHEK) then
    ...
    if CP.TypeCardNew<>1 then
     begin
      BSkd:=False;
      CP.SumOplata:=CP.SumChek;

      if CP.B1=False then
       if Not (CheckBankCard(CP)) then raise EAbort.Create('3');
      if CP.TypeOplat=TO_CHEK then
       begin
      {
...
//               if Not CheckZRep then Abort;
               ...
               if NCh<0 then raise EAbort.Create('Ошибка определения номера чека!'#10#10+
    ...
    try
     if BlockingChek=False then
      begin
       ...
       if cbEKKA.Checked then DM.WriteChek(CP)
                         else begin
 ...
 begin
  if TControl(Sender).Enabled then
   begin
    ...
    if Sender=btnB1      then PayChek(TO_CHEK,1);}
   end;
...
var db:TDBGrid;
    dR,sR:TRect;
    dx,dy,dxr,Kl,Art_Code,i,Ostat:Integer;
    PCena:String;
    tmpBColor,C:TColor;
    Srok:TDateTime;
    IsS,IsRec:Boolean;
 ...
 procedure DrawWithFilter(C,X,Y:Integer; S,F:String);
 var P:Integer;
     S1,F1:String;
  begin
  if C<>2 then Exit;
  ...
  if Length(F)>2 then
   begin
    ...
    db.Canvas.TextOut(X,Y,Copy(S,1,P-1));

    try
     db.Canvas.Brush.Color:=clYellow;
     db.Canvas.Font.Color:=clBlack;
     db.Canvas.Font.Style:=[fsBold,fsUnderline];

     db.Canvas.TextOut(X+db.Canvas.TextWidth(Copy(S,1,P-1))+1,Y,Copy(S,P,Length(F)));
     ...
     db.Canvas.TextOut(X+db.Canvas.TextWidth(Copy(S,1,P+Length(F)-1))+1,Y, Copy(S,P+Length(F),Length(S)));
    end;
   end else db.Canvas.TextOut(X,Y,S);
  end;
  ...
  if FullSprTov=False then
   if (db.DataSource.DataSet.FieldByName('OSTAT').AsInteger<=0) or ((Ostat<=0) and (Ostat<>-1)) then
    begin
     ...
     if db.DataSource.DataSet.FieldByName('F_NDS').AsInteger>2 then db.Canvas.Brush.Color:=clPurple
                                                               else db.Canvas.Brush.Color:=$00FFCB5E;
...

{
  ...
  DrawWithFilter(DataCol,Rect.Left+2,Rect.Top+1,db.DataSource.DataSet.FieldByName('NAMES').AsString,edCntxSearch.Text);
  ...
  try
   ...
   if Tg=2 then
    ...
    begin
     ...
     try
      PrOtkazF.Tp:=Tg;
      PrOtkazF.Label4.Caption:='Цена аптеки: '+CurrToStrF(DM.spY_FilterSklad.FieldByName('Cena').AsCurrency,ffFixed,2)+' грн.';
      Application.ProcessMessages;
      PrOtkazF.ShowModal;
      ...
      if Flag in [1,3] then
       begin
        if Tg=1 then Cena:='0'
                else Cena:=CurrToStr(DM.spY_FilterSklad.FieldByName('Cena').AsCurrency);
  ...
  if Tg=1 then
   begin
    Kol:=GetKolFromDialog(TrimRight(Names),DefKol,Art_Code,EN_SIMPLE);
             ...
             Kol:=GetKolFromDialog(TrimRight(DM.spY_FilterSklad.FieldByName('Names').AsString),DefKol,Art_Code,EN_SIMPLE,0,0,Cap);
             Art_Code:=DM.spY_FilterSklad.FieldByName('Art_Code').AsInteger;
            end;
...
function TMainF.CheckForReoc:Boolean;
var i:Integer;
    S:String;
 begin
  try
...
{var Path,DT:String;
 ...
 begin
  {
   if (Design) or (IsServer=False) then Exit;
  try
   for i:=7 to 60 do
...
//   DM.Qr.SQL.Add('       Sum(convert(numeric(8,2),a.cena*a.kol)) as Сумма_N_1500,');
   ...
   DM.Qr.SQL.Add('group by a.art_code,b.names,c.P1,c.P2,c.P3');
...
procedure TMainF.tmWaitCommTimer(Sender:TObject);
 begin
  try
   ...
   if (Prm.AptekaID in [216,40]) or ((Prm.AptekaID=91) and (Opt.KassaID=10)) then Exit;
 ...
 begin
  try
   ...
   if DM.Qr.IsEmpty then
    begin
     MainF.MessBox('К данному перапарту нет инструкции!');
     Exit;
    end;
 ...
 begin
  ...
  try
   ...
   try
    ...
    DM.TimerQr1.SQL.Add('select top 1 *,(select top 1 users from SprUser s (nolock) where s.id_gamma=j.id_from) as users from Inform..JMess j (nolock) where IsQueue is null and DateM>getdate()-7 and (IsNull(id_dest,-1) in (-1,'+ss+IntToStr(Prm.ID_Gamma)+') ) and id not in (');
   ...
   DM.Qr.Close;
{
   ...
   DM.Qr.SQL.Text:='select * from NaklsNeedAkt order by 3,2';
...
'where a.fname like ''FOLDER_%'' and b.id=convert(int,substring(a.fname,8,2)) ';
       DM.TimerQr2.SQL.SaveToFile('C:\Files.txt');
       DM.TimerQr2.Open;
...
//   if Not EnterCurrValue(SumNeed) then Exit;
   ...
   DM.QrToOff.ConnectionString:=Opt.ConStrMess;
...
procedure TMainF.ShowTovInfo(Art_Code:Integer; Names:String; Upak:Integer; P:Byte=0);
 begin
  try
   ...
   try
    ...
    if P=1 then TovInfoF.Expanded:=True;
    Application.ProcessMessages;
    TovInfoF.ShowModal;
   finally
    TovInfoF.Free;
...
procedure TMainF.miNoTovClick(Sender:TObject);
 begin
  if DM.spY_FilterSklad.IsEmpty then Exit;
  ShowTovInfo(DM.spY_FilterSklad.FieldByName('MakeFrom').AsInteger,'',DM.spY_FilterSklad.FieldByName('Upak').AsInteger);
 end;
...
procedure TMainF.miCompend(Sender:TObject);
 begin
  ...
  try
   DM.Qr.Close;
   DM.Qr.SQL.Text:='select Value from Spr_Const where Descr=''SiteCompend'' ';
   DM.Qr.Open;
    ...
    if AddResF.Flag=1 then
     begin
      ...
      try
       ...
       ShowText('Номер заказа: '+IntToStr(IDRes)+#10+'Собщите его клиенту он понадобится, когда клиент придет за покупкой.'+#10#10+'Бронь действительна в течении 24-x часов!');
       CancelChek;
      except
       DM.ADOCo.RollbackTrans;
       raise;
   ...
   if TWinControl(Sender).Tag=2 then
    begin
     ...
     try
      ...
      Application.ProcessMessages;
      AddResF.ShowModal;
      if Not (AddResF.Flag in [1,2]) then Exit;

      if AddResF.Flag=1 then Filt:=AddResF.Filt
                        else Filt:='';
     finally
      AddResF.Free;
     end;
   ...
   try
    ...
    if WorkResF.Flag=1 then
     begin
      ...
      if FTypeRes=1 then
       begin
        MessBox('Забронированные препараты с сайта будут проданы по сниженной цене, поэтому дисконтная карта не действует!',48,GetFont('MS Sans Serif',12,clBlue,[fsBold]),Res);
       end;
   ...
   if CheckTypeMDMU=2 then
    begin
     ...
     if EKKA.EmulEKKA then
      begin
       DM.Qr6.SQL.Add('select c.ean13,a.date_Chek,s.art_code,s.names,a.cardpref,a.skd,a.numb_chek,a.cena,convert(numeric(8,2),a.cena*(100-a.skd)/100.) as cenaskd, a.cena*Sum(a.kol) as sumbez, Sum(a.kol) as kol,'+'Sum(a.kol*a.cena)-IsNull(Sum(a.SumSkd),0) as summa,');
       DM.Qr6.SQL.Add('       IsNull((select top 1 koef from Inform..SprKoef k (nolock) where k.art_code=s.art_code),1) as koef');
       ...
       DM.Qr6.SQL.Add('group by c.ean13,a.date_chek,s.art_code,s.names,a.cardpref,a.skd,a.numb_chek,a.cena');
      end else begin
                DM.Qr6.SQL.Add('select c.ean13,getdate() as date_Chek,a.art_code,a.names,a.cardpref,a.skd,a.numb_chek,a.cena,convert(numeric(8,2),a.cena*(100-a.skd)/100.) as cenaskd, a.cena*Sum(a.kol) as sumbez, Sum(a.kol) as kol,'+'Sum(a.kol*a.cena)-IsNull(Sum(a.SumSkd),0) as summa,');
                DM.Qr6.SQL.Add('       IsNull((select top 1 koef from Inform..SprKoef k (nolock) where k.art_code=a.art_code),1) as koef');
                ...
                DM.Qr6.SQL.Add('group by c.ean13,a.art_code,a.names,a.cardpref,a.skd,a.numb_chek,a.cena');
               end;
...
//   if Design then Exit;
   ...
   DM.Qr6.Close;
   DM.Qr6.SQL.Clear;
// DM.Qr6.SQL.Add('select a.date_Chek,b.art_code,a.cardpref,a.skd,a.numb_chek,a.cena*dbo.GetKoefUpak(b.art_code),IsNull(Sum(a.SumSkd),0) as sumskd,'+' Sum(a.kol/dbo.GetKoefUpak(b.art_code)) as kol,Sum(b.ostat/dbo.GetKoefUpak(b.art_code)) as ostat');
   DM.Qr6.SQL.Add('select a.date_Chek,b.art_code,a.cardpref,a.skd,a.numb_chek,a.cena*dbo.GetKoefUpak(b.art_code) as cena,'+'IsNull(Sum(a.SumSkd),0) as sumskd,Sum(convert(numeric(8,2),a.kol)/dbo.GetKoefUpak(b.art_code)) as kol,Sum(b.ostat/dbo.GetKoefUpak(b.art_code)) as ostat ');
   DM.Qr6.SQL.Add('from ArhCheks a (nolock), SprTov b (nolock), MediCard c (nolock)');
   ...
   if DM.Qr6.IsEmpty then
    begin
     ...
     DM.Qr6.SQL.Clear;
// DM.Qr6.SQL.Add('select a.date_Chek,b.art_code,a.cardpref,a.skd,a.numb_chek,a.cena*dbo.GetKoefUpak(b.art_code),IsNull(Sum(a.SumSkd),0) as sumskd,'+' Sum(a.kol/dbo.GetKoefUpak(b.art_code)) as kol,Sum(b.ostat/dbo.GetKoefUpak(b.art_code)) as ostat');
     DM.Qr6.SQL.Add('select a.date_Chek,c.art_code,a.cardpref,a.skd,a.numb_chek,a.cena*dbo.GetKoefUpak(c.art_code) as cena,IsNull(Sum(a.SumSkd),0)'+' as sumskd,Sum(convert(numeric(8,2),a.kol)/dbo.GetKoefUpak(c.art_code)) as kol,Sum(b.ostat/dbo.GetKoefUpak(c.art_code)) as ostat ');
     DM.Qr6.SQL.Add('from ArhCheks a (nolock), SprTov b (nolock), MediCard c (nolock), Plist p (nolock)');
     DM.Qr6.SQL.Add('where a.kod_name=b.kod_name and p.art_code=b.art_code and p.makefrom=c.art_code and a.numb_chek='+IntToStr(CP.NumbChek)+' and a.vzh='+IntToStr(CP.Vzh));
     DM.Qr6.SQL.Add('group by a.date_Chek,c.art_code,a.cardpref,a.skd,a.numb_chek,a.cena');
     DM.Qr6.Open;
    ...
    if (sbMag.Down=True) and (Sender=sbMag) then
     begin
      ...
      if (Copy(IntToStr(NumCardMag),1,3)=CR_PROH) and (Date>=StrToDate('20.08.15')) and (Date<=StrToDate('30.09.15')) and (Prm.AptekaID in [21,23,26,78,138,211,230]) then
       ...
       end; // else MessBox('Данный флаер не действителен!');
 ...
 begin
  try
   if Length(edFN.Text)<>10 then
    begin
     MainF.MessBox('Фискальный номер должен быть состоять из 10 цифр');
     Exit;
    end;
...
//  if MessBox('Пациент предъявил рецепт формы Ф-1?',52)<>ID_YES then Exit;
  ...
  try
   ...
   if Not ShowQ('Присутствует ли круглая печать вашей поликлиники?',4) then Exit;
//   if Not ShowQ('Присутствует ли треугольная печать вашей поликлиники?',1) then Exit;
   if Not ShowQ('Присутствует ли прямоугольный штамп красного цвета с текстом "вартість підлягає відшкодуванню"?',3) then Exit;
   if Not ShowQ('Присутствует ли личная печать и подпись врача вашей поликлиники, который выписал данный рецепт?',5) then Exit;
...
var P,i:Integer;
 ...
 begin
  try
   ...
   if Design then Exit;
   try
    if Not (Prm.FirmID in [14,15,16,17]) then
     begin
      DM.TimerQr1.Close;
      DM.TimerQr1.SQL.Text:='select nn_nakl from TmpNakl a (nolock) where IsNull(Obl,0)=1 and not exists (select 1 from Jmoves j (nolock) where j.nn_nakl=a.nn_nakl and j.date_nakl=a.date_nakl) group by nn_nakl';
      DM.TimerQr1.Open;
      if DM.TimerQr1.IsEmpty=False then
       begin
        S:='';
        for i:=1 to DM.TimerQr1.RecordCount do
         begin
          if i=1 then DM.TimerQr1.First else DM.TimerQr1.Next;
          S:=S+DM.TimerQr1.FieldByName('nn_nakl').AsString+#10;
         end;
        ShowText('Внимание!!!'#10'Есть незагруженные накладные (по воде).'+#10#10+S+#10+' Зайдите в загрузку накладных и загрузите их в течении 15 минут.',clRed);
       end;
      end;

    DM.TimerQr1.Close;
    DM.TimerQr1.SQL.Text:='select nn_nakl from TmpNakl a (nolock) where sklad=''    16'' and not exists (select 1 from Jmoves j (nolock) where j.nn_nakl=a.nn_nakl and j.date_nakl=a.date_nakl) group by nn_nakl';
    DM.TimerQr1.Open;
    ...
    if DM.TimerQr1.IsEmpty=False then
     begin
      ...
      ShowText('Внимание!!!'#10'Есть незагруженные накладные (по расфасованным срокам).'+#10#10+S+#10+' Зайдите в загрузку накладных и загрузите их.',clRed);
     end;
...
//    if FileExists(IncludeTrailingBackSlash(Opt.LoadNaklPath)+'Nakl.txt') then
     LoadTmpNakl(DM.ADOCo,DM.Qr,IncludeTrailingBackSlash(Opt.LoadNaklPath)+'Nakl.txt',Prm.UserId);
   except
   end;
 ...
 begin
  try
   ...
   try
    ...
    QrToOff.Close;
    QrToOff.SQL.Text:='select top 1 data from datafromapteks..StampGamma (nolock) where typestamp=''osan'' ';
    QrToOff.Open;
    ...
    QrToOff.SQL.Add('(select art_code,file_path from workwith_gamma..ava_sertifikat_scan (nolock)  ');
    QrToOff.SQL.Add(' union ');
    QrToOff.SQL.Add(' select art_code,file_path from workwith_gamma..ava_sertifikat_scan_seriya (nolock)) a  ');
    ...
    if QrToOff.IsEmpty then
     begin
      MessBox('На этот препарат нет сертификата в электронном виде!');
      Exit;
     end;
...
//          PrintRep.AddText(#12)
   ...
   {  begin
    ...
    if B then PrintRep.PreView
         else MainF.MessBox('Не удалось скопировать файлы сертификатов с сервера!');
   finally
    Bm.Free;
    Bm1.Free;
    QrToOff.Free;
 ...
 begin
  ...
  try
   ...
   try
    ...
    QrToOff.CommandTimeout:=600;
    QrToOff.Close;
    QrToOff.SQL.Add('exec [192.168.0.11].datafromapteks.dbo.spY_GetSertif '+IntToStr(Art_code)+','''+NumSeriya+'''');
    QrToOff.Open;
...
procedure TMainF.edCntxSearchKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
 begin
  if (Key=VK_RETURN) and (dbgSklad.Visible) then dbgSklad.SetFocus;
 end;
...
procedure TMainF.edCntxSearchExit(Sender: TObject);
 begin
  if edCntxSearch.Text='' then imFind.Visible:=True;
  DrawRoundRect(imSearch,False,nil);
 end;
...

{
  if Sender=edVesh then
   begin
    DM.Qr.SQL.Add('select veshestvo as Descr,Len(veshestvo) as LenD into #tmpfilt from Inform..SprFormV (nolock) where veshestvo like ''%'+ss+'%'' ');
    DM.Qr.SQL.Add('and '+filt);
    DM.Qr.SQL.Add('group by veshestvo');
    DM.Qr.SQL.Add('union all ');
    DM.Qr.SQL.Add('select veshestvoen as Descr,Len(veshestvoen) as LenD from Inform..SprFormV (nolock) where veshestvoen like ''%'+ss+'%'' ');
    DM.Qr.SQL.Add('and '+filt);
    DM.Qr.SQL.Add('group by veshestvoen');
    DM.Qr.SQL.Add('order by 1');
   end else
  if Sender=edDoz then
   begin
    DM.Qr.SQL.Add('select convert(varchar,Sostav) as Descr,Len(convert(varchar,sostav)) as LenD into #tmpfilt from Inform..SprFormV (nolock) where convert(varchar,sostav) like ''%'+ss+'%'' ');
    DM.Qr.SQL.Add('and '+filt);
    DM.Qr.SQL.Add('group by Sostav');
    DM.Qr.SQL.Add('order by 1');
   end else
  if Sender=edForm then
   begin
    DM.Qr.SQL.Add('select formaen as Descr,Len(formaen) as LenD into #tmpfilt from Inform..SprFormV (nolock) where formaen like ''%'+ss+'%'' ');
    DM.Qr.SQL.Add('and '+filt);
    DM.Qr.SQL.Add('group by formaen');
    DM.Qr.SQL.Add('order by 1');
   end;
...
//  DM.Qr.SQL.SaveToFile('C:\Filter.txt');
  DM.Qr.Open;
...
function TMainF.GetIsFiltered:Boolean;
 begin
  Result:=(DM.spY_FilterSklad.IsEmpty=False) and
          ((Length(edCntxSearch.Text)>2) or (fVesh<>'') or (fDoz<>'') or (fForm<>''));
 end;
...
var C:String;
 ...
 begin
  ...
  if Tg=0 then
   begin
    if MainF.MessBox('Вы действительно хотите снять Z-отчет и сменить ставку Б на 7% ?',52)<>ID_YES then Exit;
   end else
  if Tg=1 then
   begin
    if MainF.MessBox('Вы действительно хотите снять Z-отчет и сменить ставку А на 20% ?',52)<>ID_YES then Exit;
   end;
...
procedure TMainF.N56Click(Sender: TObject);
 begin
  try
   ...
   PrintRep.Align:=AL_CENTER;
   PrintRep.AddText('Перечень медикаментов пилотного проекта '#10#10);
   ...
   DM.Qr.SQL.Add('from SprTov s (nolock), SprPilot p (nolock), SprMnn m (nolock)');
...
procedure TMainF.dbgUpDblClick(Sender: TObject);
 begin
  ...
  if dbgSklad.DataSource.DataSet.Locate('Art_Code',TDBGrid(Sender).DataSource.DataSet.FieldByName('Art_Code').AsInteger,[loCaseInsensitive,loPartialKey]) then PrevEnterKol(EK_DIALOG);
 end;
...
procedure TMainF.bbWhereIsClick(Sender: TObject);
 begin
  if DM.spY_FilterSklad.IsEmpty then Exit;
  ShowTovInfo(DM.spY_FilterSklad.FieldByName('MakeFrom').AsInteger,'',DM.spY_FilterSklad.FieldByName('Upak').AsInteger,1);
 end;
...
var Num:String;
 begin
  ...
  if TSpeedButton(Sender).Down=True then
   if Not EnterStrValue(Num,'Введите 8 последних символов с карточки ОранжКард!') then TSpeedButton(Sender).Down:=False
   else begin
         FNumGeptralCard:=Num;
...
procedure TMainF.BitBtn9Click(Sender: TObject);
 begin
  TryWriteToJournal;
