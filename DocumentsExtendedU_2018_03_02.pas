...
begin
  ...
  begin
    try
      Connection:=DM.ADOCo;
      SQL.Clear;
      SQL.Text:='select ID from INFORM.dbo.FOLDERS where DESCR like '''+Node.Text+'''';
      Open;
      if not IsEmpty then nodeId:=FieldByName('ID').AsInteger else nodeId:=-1;
    finally
      Free;
    end;
  end;
...
begin
  try
    ...
    DM.qrFiles.SQL.Add('  f.ID, SubString(fname, 11, Len(f.FName)) as Fname');
    DM.qrFiles.SQL.Add('from');
    DM.qrFiles.SQL.Add('  INFORM.dbo.JFILES f');
    DM.qrFiles.SQL.Add('  left join INFORM.dbo.FOLDERS d (nolock) on d.ID=f.SubFolderID');
    DM.qrFiles.SQL.Add('where');
    DM.qrFiles.SQL.Add('  f.FName like ''FOLDER[_]'+IntToStrF(folderId,2)+'[_]%''');
