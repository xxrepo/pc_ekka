        ...
        Open;
{
        ...
        SQL.Add('  apteka_net.dbo.GetNamesUpak(p.names) as Names, p.koef, p.manufacturer,');
        SQL.Add('  sum(r.kol) as kol, max(s.CenaZakup) as cena, r.pr_vz_post,');
...
//    SQL.Add('declare @cenazakup numeric(8,2) set @cenazakup=convert(numeric(8,2),:cenazakup)');
    SQL.Add('declare @art_code int set @art_code = convert(int,:art_code)');
    SQL.Add('select');
    SQL.Add('  j.type_nakl, s.art_code, j.F_NDS, j.id_postav, j.nn_prih, j.dt_prih, j.iddoc, sum(r.kol) as kol, max(s.CenaZakup) as CenaZakup, max(s.Cena) as Cena');
...
//    SQL.Add('  and s.CenaZakup=@cenazakup');
...
//    Parameters.CreateParameter('cenazakup',ftFloat,pdInput,11,ADOQuery1.FieldByName('cenazakup').Value);
{
...
//      Tb.Cell[6,row_no].Align:=AL_CENTER; Tb.Cell[6,row_no].AddText( CurrToStr( RoundCurr(ADOQuery1.FieldByName('cena').Value / ADOQuery1.FieldByName('koef').Value) )); //Цена (грн.)
      ...
      Tb.Cell[8,row_no].Align:=AL_CENTER; Tb.Cell[8,row_no].AddText(FloatToStr(StrToFloat(FloatToStrF(ADOQuery2.FieldByName('kol').Value / ADOQuery2.FieldByName('koef').Value - ADOQuery1.FieldByName('kol').Value / ADOQuery1.FieldByName('koef').Value,10,3))));
...
//                      Break;
    ...
    begin
      ...
      begin
        ...
        SQL.Add('declare @iddoc int set @iddoc = :iddoc');
        SQL.Add('select');
        SQL.Add('  j.iddoc, j.F_NDS, r.KOL, s.CenaZakup as CenaZakup, s.CENA as CENA, p.koef');
