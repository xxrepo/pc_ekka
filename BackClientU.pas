...
uses
  ...
  MainU, SysUtils, Windows, EKKAU, Util, ShowTextU, ComCtrls, ComObj;
//  , Variants, Graphics, PrintReport, Dialogs;
...
uses
  DataModuleU, ShareU, ChekListU, FormForVozrU, EnterValueU;
...
//             end;
   ...
   if FiSStrah then
    begin
     CP.Fransh:=DM.qrBack.FieldByName('fransh').AsCurrency;
     CP.FranshStr:=DM.qrBack.FieldByName('franshstr').AsCurrency;
...
//     CP.dt_dostav:=DM.qrBack.FieldByName('dt_dostav').AsDateTime;
   ...
   CP.TypeChek:=DM.qrBack.FieldByName('TypeChek').AsInteger;
   CP.SumB3:=DM.qrBack.FieldByName('SumB3').AsCurrency;
...
//        if EKKA.ReceiptNumber<0 then raise EAbort.Create('Ошибка определения номера чека');
        if EKKA.ReceiptNumber(1)<0 then raise EAbort.Create('Ошибка определения номера чека');
        CP.NumbChek:=EKKA.ReceiptNumber(1)+1;
...
//      ShowMessage(CurrToStr(CP.SumChek)+' | '+IntToStr(Ty)+' | '+CurrToStr(CP.SumDopl));
      ...
      if (CP.IsMed) and (CP.SumChek-CP.SumDopl>0) then
       begin
        ...
        EKKA.fpCloseFiscalReceipt(CP.SumDopl,Ty,CP.SumChek,CP.SumChek-CP.SumDopl,False,0,StrToIntDef(MainF.RRN,1),MainF.BankCard);
       end else
      if (CP.IsStrah) and (CP.SumChek-CP.Fransh>0) and (CheckBox2.Checked=False) then
       begin
        ...
        EKKA.fpCloseFiscalReceipt(CP.Fransh,Ty,CP.SumChek,CP.SumChek-CP.Fransh,False,0,StrToIntDef(MainF.RRN,1),MainF.BankCard);
       end else
...
//        ShowMessage(CurrToStr(CP.SumChek)+' | '+CurrToStr(CP.SumChekRecipt));
        ...
        EKKA.fpCloseFiscalReceipt(0,Ty,CP.SumChek,CP.SumChekRecipt,True,0,StrToIntDef(MainF.RRN,1),MainF.BankCard);
       end else
      if CP.SumSpisDnepr>0 then
       begin
        ...
        if CP.SumChek>CP.SumSpisDnepr then
         begin
          ...
          if Not EKKA.fpCloseFiscalReceipt(CP.SumChek,Ty,CP.SumChek,CP.SumSpisDnepr,True,0,StrToIntDef(MainF.RRN,1),MainF.BankCard) then AbortM('Ошибка закрытия чека: '+EKKA.LastErrorDescr+' | '+EKKA.LastError)
         end else begin
                   ...
                   if Not EKKA.fpCloseFiscalReceipt(0,Ty,CP.SumChek,CP.SumChek,True,0,StrToIntDef(MainF.RRN,1),MainF.BankCard) then AbortM('Ошибка закрытия чека: '+EKKA.LastErrorDescr+' | '+EKKA.LastError);
                  end;
                 ...
                 if Not EKKA.fpCloseFiscalReceipt(CP.SumChek,Ty,CP.SumChek,0,false,0,StrToIntDef(MainF.RRN,1),MainF.BankCard) then AbortM('Ошибка закрытия чека: '+EKKA.LastErrorDescr+' | '+EKKA.LastError);
                end;
 ...
 //    DM.ADOCo.CommitTrans;
       ...
       if (FIsTerminal=False) and (Prm.IsKurier=False) and (Prm.OptCena=False) and (FIsStrah=False) and (FIsIApteka=False) then
        begin
...
Begin
  ...
  begin
    if not EnterStrValue(RRN,'Вы действительно хотите совершить возврат по терминалу?') then Exit;//<>ID_YES then
  end;
  MainF.RRN:=RRN;
  try
    try
        ...
        try
          ...
          begin
            if CheckBox1.Checked then
              ...
              Sum:=Round(StrToCurr(lbSum.Caption)*100);
            Term.Refund(Sum,0,StrToIntDef(Term.MerchantID,1),MainF.RRN);
          end;
          //IsTerminate:=False;

          WaitResponse(oleVariant(Term));
          if length(trim(MainF.BankCard)) <= 0 then MainF.BankCard:='000000000000000';
