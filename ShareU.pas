...
var Ty:Integer;
 begin
  ...
  try
   ...
   if Not EKKA.fpCloseFiscalReceipt(0,Ty,0,0,false,0,StrToIntDef(MainF.RRN,1),MainF.BankCard) then AbortM(EKKA.LastErrorDescr);
...
//   ShowMessage(EKKA.RD_Item.Text);
   ...
   CP.Sums[12]:=StrToInt64(EKKA.RD_Item[3])*0.01;   // Продажи по ставке В0%
   CP.Sums[13]:=StrToInt64(EKKA.RD_Item[4])*0.01;   // Продажи по ставке Г0%
   CP.Sums[14]:=StrToInt64(EKKA.RD_Item[13])*0.01;  // Возвраты от покупателей по ставке В0%
   CP.Sums[15]:=StrToInt64(EKKA.RD_Item[14])*0.01;  // Возвраты от покупателей по ставке Г0%
   ...
   try
    ...
    Case P of
                ...
                begin
                 ...
                 Exit;
                          ...
                          if
                             ...
                             (Prm.AptekaID=292) or
                             (Prm.AptekaID=279) OR
                             (Prm.AptekaID=0) then begin Result:=1; Exit; end; // !!!!!!!!!!!!!!!!!! В будущем обязательно убрать.    // 14072018
     ...
     ST_ZREP:begin
              ...
              if ((EKKA.TypeEKKA=EKKA_MARRY301MTM) {or (EKKA.TypeEKKA = EKKA_E810T)} or ((EKKA.TypeEKKA=EKKA_N707)) or (CP.NumZ=-1)) and (P1=0) then
               begin
...
//       if Prm.AptekaID in [3,68,69,70,71,72,73,75,76] then Nds:=2;
       ...
       if EKKA.TypeEKKA in [EKKA_EXELLIO,EKKA_FP2000,EKKA_N707] then Ac:=Qr.FieldByName('Art_Code').AsInteger else
       if EKKA.TypeEKKA=EKKA_E810T then
        begin
         Ac:=StrToInt(Qr.FieldByName('Art_Code').AsString+Copy(IntToStr(Random(888)+100),1,3));
        end else Ac:=0;
...
{                EKKA.fpServiceText(1,1,0,'               ');
                ...
                if EKKA.TypeEKKA<>EKKA_MARRY301MTM then
                 begin
                  ...
                  else if EKKA.TypeEKKA=EKKA_E810T then
                  begin
                    EKKA.fpServiceText(1,1,0,Copy(Prm.ZavApteki,1,Pos(' ',Prm.ZavApteki)-1));
                    EKKA.fpServiceText(1,1,0,'- - - - - - - - - - - - - -');
                  end else
                  ...
                  if Prm.Region<2 then
                   begin
                    ...
                    begin
                      if EKKA.TypeEKKA=EKKA_E810T then
                      begin
                        EKKA.fpServiceText(1,1,0,'Поиск и резервирование');
                        EKKA.fpServiceText(1,1,1,'лек-ств 0-800-505-911');
                      end
                      else
                      begin
...
//   if MainF.CheckBox2.Checked then EKKA.fpCancelFiscalReceipt else
    begin
     TS:=0;
     if (CP.TypeChek in TC_DL) {and (CP.Fransh>0)} and (CP.SumChek-CP.SumDopl>0) then
      begin
       //EKKA.fpCloseFiscalReceipt(CP.SumDopl,Ty,SumChek,SumChek-CP.SumDopl,False);
       EKKA.fpCloseFiscalReceipt(CP.SumDopl,Ty,SumChek,SumChek-CP.SumDopl,False,0,StrToIntDef(MainF.RRN,1),MainF.BankCard);
      end else
...
//       ShowMessage(IntToStr(Ty)+' - '+CurrToStr(SumChek)+' - '+CurrToStr(CP.SumChekRecipt));
       ...
       EKKA.fpCloseFiscalReceipt(TS,Ty,SumChek,CP.SumChekRecipt,True,0,StrToIntDef(MainF.RRN,1),MainF.BankCard);
     ...
     if MainF.TypeChek in [TC_SERTIF,TC_STRETCH] then
      begin
       ...
       if CP.SumChek>CP.SumSertif then EKKA.fpCloseFiscalReceipt(TS,Ty,SumChek,CP.SumSertif,IsDnepr,0,StrToIntDef(MainF.RRN,1),MainF.BankCard)
                                  else EKKA.fpCloseFiscalReceipt(0,Ty,SumChek,CP.SumSertif,IsDnepr,0,StrToIntDef(MainF.RRN,1),MainF.BankCard);
      end else begin
                ...
                if CP.TypeOplat=TO_CHEK then EKKA.fpCloseFiscalReceipt(SumChek,Ty,SumChek,0,false,0,StrToIntDef(MainF.RRN,1),MainF.BankCard)
                                        else EKKA.fpCloseFiscalReceipt(CP.TakedSum,Ty,SumChek,0,false,0,StrToIntDef(MainF.RRN,1),MainF.BankCard);
               end;
    end;
   finally
