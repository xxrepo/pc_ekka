...
(*
  ...
  @abstract( Contйm todas as classes para manipular o formato JSON. )

  As principais classes sгo :  TJSONObject,  TJSONArray .

  OBS: A unit uJSON foi adaptada de uma implementaзгo para json em Java ver
  http://www.json.org .
...
const
//  CROTINA_NAO_IMPLEMENTADA :string = 'Rotina Nгo Implementada';
  CROTINA_NAO_IMPLEMENTADA :string = 'Не реализовано';
...
//              result + '.');
      raise syntaxError('Ожидается ' + c + ' но увидел ' +
              result + '.');
  end;
end;
...
//      raise syntaxError('Substring bounds error');
      raise syntaxError('Ошибка границ подстроки');
   end;
...
//                            raise syntaxError('Unclosed comment.');
                            raise syntaxError('Не закрытый комментарий.');
                        end;
                        if (c = '*') then begin
                            if (next() = '/') then begin
                                break;
...
//                raise syntaxError('Unterminated string');
                raise syntaxError('Не завершенная строка');
            end;
            '\': begin
                c := next();
                case (c) of
...
//            raise syntaxError('Missing value.');
            raise syntaxError('Отсутствует значение.');
        end;
...
//      raise x.syntaxError('A TJSONObject must begin with "{"');
      raise x.syntaxError('TJSONObject должен начинаться с "{"');
  end;
  while (true) do begin
      c := x.nextClean();
      case (c) of
...
//          raise x.syntaxError('A TJSONObject must end with "}"');
          raise x.syntaxError('TJSONObject должен заканчиваться с "}"');
      '}': begin
          exit;
      end
      else begin
...
//          raise x.syntaxError('Expected a ":" after a key');
          raise x.syntaxError('Отсутствует ":" после ключа');
      end;
...
//          raise x.syntaxError('Expected a "," or "}"');
          raise x.syntaxError('Отсутствует "," или "}"');
      end
      end;
  end; //while
...
//            raise NullPointerException.create('Null key');
            raise NullPointerException.create('Пустое значение');
   end else begin
...
begin
   put(key, _Integer64.create(value));
   result := self;
end;
...
//   inherited createFmt(_message+#10#13' erro no caracter : %d',[index]);
   inherited createFmt(_message+#10#13' ошибка в символе : %d',[index]);
end;
...
//      raise x.syntaxError('A TJSONArray must start with "["');
      raise x.syntaxError('TJSONArray должен начинаться с "["');
  end;
  if (x.nextClean() = ']') then begin
      exit;
  end;
...
function _Integer64.clone: TZAbstractObject;
begin
  result := _Integer64.create (self.fvalue);
end;

constructor _Integer64.create(i: Int64);
begin
  fvalue := i;
end;

constructor _Integer64.create(s: string);
begin
   fValue := StrToInt64(s);
end;

constructor _Integer64.create(s: _String);
begin
   Create(s.toString);
end;

function _Integer64.doubleValue: double;
begin
  Result := fvalue;
end;

function _Integer64.int64Value: Int64;
begin
  Result := fValue;
end;

function _Integer64.intValue: Integer;
begin
  Result := 0;
end;

class function _Integer64.toHexString(c: char): string;
begin
  result := IntToHex(ord(c),2);
end;

function _Integer64.toString: string;
begin
  result := intToStr(fvalue);
end;
