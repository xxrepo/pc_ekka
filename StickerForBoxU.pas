...
type
  ...
  private
    function beginStrLog: string;
  public
    StickerForBox: TextFile;
  end;
...
const
  ...
  LB_DESCR_CAPTION = 'Описание содержимого в ящике';
  LB_COUNT_CAPTION = 'Количество позиций';
  LB_BOX_NO_CAPTION = '№ ящика';

var StickerForBoxF:TStickerForBoxF;
...
begin
  ...
  if FileExists(Opt.LogPath+'\StickerForBox.log') then
  begin
    AssignFile(StickerForBox,Opt.LogPath+'\StickerForBox.log');
    Reset(StickerForBox);
  end
  else
  begin
    AssignFile(StickerForBox,Opt.LogPath+'\StickerForBox.log');
    Rewrite(StickerForBox);
  end;
  Append(StickerForBox);
  Writeln(StickerForBox);
  Writeln(StickerForBox);
  Writeln(StickerForBox,'********************************************************************************');
  Writeln(StickerForBox,beginStrLog+' открыл форму редактирования');
end;
...
var
  Tb: TTableObj;
begin
  Writeln(StickerForBox,beginStrLog+' заполнил форму редактирования:');
  Writeln(StickerForBox,LB_APTEKA_CAPTION+' '+edApteka.Text);
  Writeln(StickerForBox,LB_DATE_TIME_CAPTION+' '+FormatDateTime('YYYY-MM-DD',dtDate.Date)+' '+FormatDateTime('HH:MM',dtTime.Time));
  Writeln(StickerForBox,LB_FIO_CAPTION+' '+edFIO.Text);
  Writeln(StickerForBox,LB_DESCR_CAPTION+' '+meDescr.Text);
{
  Writeln(StickerForBox,LB_COUNT_CAPTION+' '+edCount.Text);
  Writeln(StickerForBox,LB_BOX_NO_CAPTION+' '+edBoxNo.Text);
}
  try
    PrintRep.Clear;
    PrintRep.SetDefault;
...

{
    ...
    Writeln(StickerForBox,beginStrLog+' открыл превью формы');
    if PrintRep.PreView then
      Writeln(StickerForBox,beginStrLog+' отправлено на печать')
    else
      Writeln(StickerForBox,beginStrLog+' не было напечатано');
  except
    on E:Exception do
    begin
      MainF.MessBox('Ошибка формирования печатной формы накладной: '+E.Message, 16);
      Writeln(StickerForBox,beginStrLog+' Ошибка формирования печатной формы накладной: '+E.Message);
    end;
  end;
end;
...
procedure TStickerForBoxF.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Writeln(StickerForBox,beginStrLog+' закрыл форму редактирования ***');
  CloseFile(StickerForBox);
end;

function TStickerForBoxF.beginStrLog: string;
begin
  result:='*** '+formatDateTime('YYYY-MM-DD HH:MM:SS',Now())+' '+IntToStr(Prm.ID_Gamma)+' '+Prm.UserName+' ';
end;
