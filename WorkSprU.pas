...
uses
  ...
  {Dialogs, }ExtCtrls, StdCtrls, Buttons, Grids, DBGrids, Menus, DB,
  PrintReport, Util, Types, ADODB;
...
type
  ...
  TWorkSprF = class(TForm)
    ...
    procedure bbPrintACennikClick(Sender: TObject);
...
uses MainU, DataModuleU, ShowKolForVozrU, DateBackU, ClDiagU, PrVozrU, PrintMarkBoxU, EnterValueU, ShareU,
     AddResU,ShowTextU, BackSkladInfoU, ZakazPriceU, QuarantineU;
...
//  if Not (P in [W_ZAKAZSKL,W_ZAKAZWAIT,W_QUARANTINE1]) then
...
//    if P in [W_BACK,W_BACK1,W_QUARANTINE1] then
     begin

//      if p <> W_QUARANTINE1 then
//      begin
        DM.Qr.Close;
...
//                               WorkSprF.btPrCennikAkcia15.Visible:=False;
     ...
     W_LT,W_LTM:begin
                 WorkSprF.bbLt.Visible:=True;
                 WorkSprF.bbLt.Enabled:=True;
                end;
   ...
   if Flag<>W_Back then
    begin
     ...
     if Flag=W_BACK1 then
      DM.qrSpr.SQL.Add('Sum(ostat_)-IsNull((select Sum(kol) from Inform..Tmp_Spis ttt where ttt.art_code=a.art_code and ttt.Priznak='+IntToStr(Flag)+'),0) as Kol,Max(Cena_) as Cena,IsNull((select Max(n.Art_Code) from NoScan n where n.art_code=a.art_Code),0) as IsScan ') else
     if Flag=W_LTM then
      begin

       DM.qrSpr.SQL.Add('Sum(ostat_)+IsNull((select Sum(kol) from Inform..Tmp_Spis ttt where ttt.art_code=a.art_code and ttt.Priznak='+IntToStr(Flag)+'),0)');
       ...
       DM.qrSpr.SQL.Add('Max(Cena_) as Cena, IsNull((select Max(n.Art_Code) from NoScan n where n.art_code=a.art_Code),0) as IsScan ')

      end else DM.qrSpr.SQL.Add('Sum(ostat_) as Kol,Max(Cena_) as cena,IsNull((select Max(n.Art_Code) from NoScan n where n.art_code=a.art_Code),0) as IsScan ');
     ...
     if Flag=W_LTM then
      begin
       DM.qrSpr.SQL.Add('from SprTovar a left join Inform..SprReg r on a.art_code=r.art_code where ostat_<0 ');
      end else DM.qrSpr.SQL.Add('from SprTovar a left join Inform..SprReg r on a.art_code=r.art_code where 1=1 ');
//               DM.qrSpr.SQL.Add('from SprTov a where art_code not in (select art from tmpa) ');
     if Flag=W_CENNIK then
      DM.qrSpr.SQL.Add(' and a.names not like ''%SHEFER%''  ');

     if cbZero.Checked then
      DM.qrSpr.SQL.Add(' and a.ostat_>0  ');
     if Flag=W_CENNIKSROK then
      DM.qrSpr.SQL.Add(' and a.art_code in (select askd.art_code from AddSkd askd where getdate() between DtStart and DtEnd) and a.ostat_>0 and a.names like ''%([0-9][0-9].[0-9][0-9])%''  ');
...
//                DM.qrSpr.SQL.Add(' and (a.art_code not in (95265,144863)  ');
     ...
     if Flag=W_LTM then
      begin
       DM.qrSpr.SQL.Add('having Sum(ostat_)+IsNull((select Sum(kol) from Inform..Tmp_Spis ttt where ttt.art_code=a.art_code and ttt.Priznak='+IntToStr(Flag)+'),0)');
     ...
     if Flag in [W_LT] then
      begin
       ...
       DM.qrSpr.SQL.Add('where p.art_code not in (select art_code from SprTovar) and  ');
...
//      DM.qr.SQL.SaveToFile('C:\987.txt');
 ...
 //    DM.Qr.SQL.Add('if (select Sum(ostat) from SprTov (nolock) where art_code='+IntToStr(Art_Code)+')>='+IntToStr(Kol*Koef)+' select 0 as Res else select 1 as Res');
     ...
     DM.Qr.SQL.Add(IntToStr(Kol*Koef)+','+CurrToStr(Ds.FieldByName('Cena').AsCurrency)+','+IntToStr(FUserID)+','+IntToStr(Flag)+',3,'+IntToStr(Scan)+',host_name(),'+IntToStr(PrVozr)+',');
...
var CP:TChekPos;
    UI:Integer;
    s:String;
    CountOfEmptySeria: integer;
    PriorIDDOC, LastIDDOCs: integer;
 begin
  //получить номер последней возвратной накладной до формирования возврата по Ф.И.
  PriorIDDOC:=0; //номер последней возвратной накладной до формирования возврата по Ф.И.
  LastIDDOCs:=0; //номер последней возвратной накладной после формирования возврата по Ф.И.
  with DM.Qr1 do
  begin
    Close;
    SQL.Clear;
    SQL.Add('select  ');
    SQL.Add('  max(IDDOC) as MaxIDDOC ');
    SQL.Add('from ');
    SQL.Add('  JMOVES ');
    SQL.Add('where ');
    SQL.Add('  TYPE_NAKL = 2');
    Open;
    PriorIDDOC:=FieldByName('MaxIDDOC').AsInteger;
  end;
...
//  if (Flag=W_BACK1)or(Flag=W_QUARANTINE1) then
  if Flag=W_BACK1 then
   try
...
//   CountOfEmptySeria:=0; //количество препаратов в карантине с не заполненными серией и / или сроком годности
//   if ((Flag=W_BACK1)and(WorkSprF.PrVozr=5))or((Flag=W_QUARANTINE1)and(WorkSprF.PrVozr=5)) then
//   if (Flag=W_BACK1)and(WorkSprF.PrVozr=5) then
//     try
//       with DM.qrQuarantine do
//       begin
//         Close;
//         SQL.Text:='exec spY_AddToQuarantine '+IntToStr(CP.ID_User)+', '+IntToStr(Prm.ID_Gamma);
//         Open;
//         CountOfEmptySeria:=FieldByName('CountOfEmptySeria').AsInteger;
//       end;
//     except
//       on E:Exception do Raise EAbort.Create('Ошибка заполнения карантина: '+E.Message);
//     end;
//
   DM.WriteChek(CP);

   DM.Qr1.Close;
   DM.Qr1.SQL.Text:='delete from Inform.dbo.Tmp_Spis where priznak='+IntToStr(Flag)+' and id_user='+IntToStr(FUserID);
   DM.Qr1.ExecSQL;
...
//   DM.ADOCo.CommitTrans;
   ...
   //отображение информации о сформированых возвратных накладных и товарах, попавших в карантин
   if (CP.Priznak = 5) and (Flag = W_BACK1) then
   try
     ...
     begin
       ...
       SQL.Add('  TYPE_NAKL = 2');
       Open;
       LastIDDOCs:=FieldByName('MaxIDDOC').AsInteger;
     end;
...
{
   if CountOfEmptySeria > 0 then
   begin
     WorkSprF.Close;
     MainF.MessBox('В карантине присутствуют товары с не указаными серией и/или сроком годности!',64);
     MainF.miQuarantineClick(sender);
   end
   else
}
     Close;
...
procedure TWorkSprF.bbPrintACennikClick(Sender: TObject);
 begin
  DM.QrEx.Close;
  DM.QrEx.SQL.Clear;
  DM.QrEx.SQL.Add('select a.art_code ');
  DM.QrEx.SQL.Add('from vAkciiArtList a ');
  DM.QrEx.SQL.Add('      inner join ');
  DM.QrEx.SQL.Add('     SprTovar t (nolock) on a.art_code=t.art_code ');
  DM.QrEx.SQL.Add('where t.ostat_>0 ');
  DM.QrEx.Open;
  if DM.QrEx.IsEmpty then Exit;
  PrintCennikListExt(DM.QrEx,Flag,CheckBox2.Checked,False,False,1);
 end;
