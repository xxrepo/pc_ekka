      ...
      begin
{
        ...
        SQL.Add('  sum(r.kol) as kol, max(r.CenaZakup) as cena, r.pr_vz_post,');
//        SQL.Add('  sum(r.kol) as kol, max(r.cena) as cena, r.pr_vz_post,');
        ...
        SQL.Add('  p.names, p.koef, p.manufacturer, r.pr_vz_post, post.descr, post.adres, i.numseriya, i.SrokGodn, vz.descr, r.VzDescr');
        SQL.Add('order by');
        SQL.Add('  j.iddoc, j.nn_nakl');
        Parameters.Clear;
        Parameters.CreateParameter('iddoc',ftInteger,pdInput,10,DM.spY_NaklList.FieldByName('iddoc').Value);
...
begin
  ...
  begin
    ...
    SQL.Add('select');
    SQL.Add('  j.type_nakl, s.art_code, j.F_NDS, j.id_postav, j.nn_prih, j.dt_prih, j.iddoc, sum(r.kol) as kol, max(r.CenaZakup) as cena');
...
//    ShowMessage(DM.spY_NaklList.Parameters.ParamByName('@refresh').Value);
