...
Uses
  ...
  ClaimesU, ViewLessonsU, FarmZamAndSoputstvU;
...
{
   if FTypeChek=TC_PILOT then DM.spY_FilterSklad.Parameters.ParamValues['@IsPilot']:=1
                         else DM.spY_FilterSklad.Parameters.ParamValues['@IsPilot']:=0;
...
var tmp,Ostat,CKol,{AC,} PkolAdd,KolD,i,Upak,Koef,Koef1,Kol,KolAdd,ArtAdd,Art_Code,Art_CodeSafe,Old_Art,F_NDS,Type_Map,i1:Integer;
    Srok,T:TDateTime;
  ...
  begin
{
...

 {
   ...
   if Not ( (FID_strah>0) or (TypeChek=TC_PILOT)) then
...

{
     ...
     for i:=1 to 100000 do
      begin
       if i>1 then
        begin
         if (Fid_strah>0) or (TypeChek=TC_PILOT)  or (Param=EK_SILENT) then Break;
         Ep.id_akcii:=0;
         Ep.id_prior:=0;
         if i=2 then
          begin
       ...
       try
        ...
        if (FId_strah>0) or (Param=EK_SILENT) then DM.spY_EnterKol.Parameters.ParamValues['@simp']:=1
    ...
    if DM.TimerQr.FieldByName('Cnt').AsInteger>0 then
     try
      ...
      try
       ...
       if ((T<StrToTime('23:45:00')) or (T>StrToTime('00:15:00'))) and (Prm.AptekaID<>240) then
        if (B) and (MaxIDRES<DM.TimerQr.FieldByName('IDRES').AsInteger) then
         MessBox('На Вашей аптеке только что забронировали товары. Для их просмотра, нажмите на мигающую кнопку в правом верхнем углу экрана!',
                 48,GetFont('MS Sans Serif',12,clBlue,[fsBold]),20,Res);
...
//   if ((T>StrToTime('03:00:00')) and (T<StrToTime('03:00:08'))) then Close;
...
//   CP.NumMediCard:=StrToInt64('272'+Copy(IntToStr(NumMediCard),4,10));
   ...
   CP.SumDopl:=0;
   if TypeChek=TC_PILOT then CP.SumDopl:=FSumDopl;
  ...
  begin
   ...
   if (CP.TypeOplat in [TO_CASH,TO_CHEK,TO_CHP]) or ( (TypeChek in [TC_SERTIF,TC_STRETCH]) ) then
    ...
    if NumCardDnepr<>-1 then
     ...
     try
      if (CP.id_strah>0) or (CP.TypeChek=TC_PILOT) then
       begin

        if CP.id_strah>0 then SumDopl:=CP.Fransh else
        if CP.TypeChek=TC_PILOT then SumDopl:=CP.SumDopl;
  ...
  if (Prm.AptekaID=232) and (CP.TypeOplat=TO_CHEK) then
    ...
    if CP.TypeCardNew<>1 then
     begin
      ...
      if CP.TypeChek=TC_PILOT then CP.SumOplata:=CP.SumDopl
...
//                DM.QrEx.SQL.SaveToFile('C:\JournZSums.txt');
     ...
     if (CP.id_strah>0) or (CP.TypeChek=TC_PILOT) then EKKA.fpCheckCopy;
...
222324
222325
insert into skd_limit(art_code,type_skd,skd,fix,dtstart,dtend) values(222322,30,99.99,1,'2018-02-01','2018-03-31 23:59:59')


delete from type_skd=40


insert into skd_limit(art_code,type_skd,skd,fix,dtstart,dtend) values(239424,40,20,1,'2018-02-09','2018-02-15 23:59:59')


insert into skd_limit(art_code,type_skd,skd,fix,dtstart,dtend) values(67475 ,40,20,1,'2018-02-09','2018-02-15 23:59:59')
