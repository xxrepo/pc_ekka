...
// Проверка корректности номера карточки
...
var ln:Integer;
 begin
  try
   ...
   Result:=0;
   ln:=Length(S);
   if (CR_BEGIN=Copy(S,1,Length(CR_BEGIN))) and (ln=13) then Result:=1 else // 1 - зеленые карточки (наши)
   if (CR_BEGDR=Copy(S,1,Length(CR_BEGDR))) and (ln=13) then Result:=2 else // 2 - карточки 10% на определенные препараты
   if (CR_NAKLOF=Copy(S,1,Length(CR_NAKLOF))) and (ln=13) then Result:=7 else // 2 - карточки 10% на определенные препараты
   ...
   if ln=8 then Result:=4                                                    // 4 - Оранж кард
  except
   Result:=0;
  end;
 end;
...
function TCardReadF.NumCardToInt(S:String; Pref:String):Int64;
 begin
  if (Pref=CR_BEGDR) or (Pref=CR_NAKLOF)  then
   begin
    S[7]:=S[4];
    S[6]:=Pref[3];
   end;
  S[1]:='0'; S[2]:='0'; S[3]:='0'; S[4]:=Pref[1]; S[5]:=Pref[2];
...
{         Case KodEAN of
          1:NumCard:=StrToInt(sNC);
          5:NumCard:=NumCardToInt(sNC,CR_911);
         end; } 
