...
procedure TJPaySlipF1.BitBtn2Click(Sender: TObject);
var r1,r2,r3,r4,r5,r6,r7,ind,CA,i,j,q,y,rn1,rn2,rn3,rn4,rn5,rn6,rn7,rn8,r8,r2_,r2__,r9,rn9:Integer;
    Tb:TTableObj;
    SumPPM,Sm,Sum1,Sum2,Sum3,SumN1,SumN2,SumN3,SumN4,SumN4_,SumN5,SumN6,SumH,SumV,SumD,SumR,SumN_1,SumN_2,SumN_3,SumN_4,SumN_5,SumN_8,SumN_9:Currency;
 ...
 begin
  ...
  c3:=3; // Номер колонки с которой начинаются данные
  if DM.qrJPaySlip1.IsEmpty then Exit;
  DM.Qr.SQL.Clear;
  DM.Qr.SQL.Add('select * ');
  DM.Qr.SQL.Add('from vZPLIST_VIEW (nolock) ');
...
//  PrintRep.AddText('Аптека: '+DM.Qr.FieldByName('Организция').AsString+' '+DM.Qr.FieldByname('Аптека').AsString+#10#10);
  ...
  PrintRep.AddTable(DM.Qr.RecordCount+c3-1,High(A)+7);
...
//  Tb.Cell[3,1].Font.Size:=4; Tb.Cell[3,1].Align:=AL_CENTER; Tb.Cell[3,1].AddText('Методика расчета');
  ...
  r1:=-1;  r2:=-1; r3:=-1; r4:=-1; r5:=-1; r6:=-1; r2_:=-1; r2__:=-1;
  r7:=-1; r6:=-1;   rn1:=-1; rn2:=-1; rn3:=-1; rn4:=-1; rn5:=-1; rn6:=-1; rn7:=-1; rn8:=-1; r8:=-1;
...
//  Tb.MergeCells(2,r1,2,r1+1);
  ...
  if r2_<>-1 then Tb.MergeCells(c3,r2_,3+DM.Qr.RecordCount,r2_);
  if r2__<>-1 then Tb.MergeCells(c3,r2__,3+DM.Qr.RecordCount,r2__);
...
//    PrintRep.AddText('ФИО: '+DM.Qr.FieldByname('ФИО').AsString+#10#10);
    ...
    for j:=Low(A) to High(A) do
     begin
      ...
      if (y+1=rn1) or (y+1=rn2) or (y+1=rn3) or (y+1=rn4) or (y+1=rn5) or (y+1=rn6) or (y+1=rn7) or (y+1=rn8) or (y+1=rn9) or (y+1=r2_) or (y+1=r2__) or (y+1=rp2) or (y+1=r8) or (y+1=r9) then
      ...
      if (AnsiLowerCase(DM.Qr.Fields[A[j]].FieldName)<>AnsiLowerCase('Удержания итого')) and
         ...
         (AnsiLowerCase(DM.Qr.Fields[A[j]].FieldName)<>AnsiLowerCase('Корректировка окладной части'))
...
//    Sum3:=Sum3+DM.Qr.FieldByName('Заработная плата').AsCurrency;
    ...
    SumPrem:=SumPrem+DM.Qr.FieldByName('Премия за коллективную ответственность').AsCurrency+
                    ...
                    +DM.Qr.FieldByName('Премия за культуру обслуживания').AsCurrency+
...
//  DM.qrJPaySlip.SQL.Add(' and id_apteka='+IntToStr(Prm.AptekaID));
  ...
  for i:=1 to DM.qrJPaySlip.RecordCount do
   begin
    ...
    DM.Qr.SQL.Add('order by numstr,param ');
//    DM.Qr.SQL.SaveToFile('C:\PaySlip.txt');
    DM.Qr.Open;
...
//  Tb.Cell[c3,y+10].AddText(CurrToStrF(Sum2-({Sum1+SumN1+SumN2+SumN3+SumN4+SumN5+SumN6+}SumN4_),ffFixed,2));
  Tb.Cell[c3,y+10].AddText(CurrToStrF(Sum2-(Sum1+SumN_8+SumN_9),ffFixed,0));
