...
Uses OpenLib, Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms, Dialogs,
     ...
     RpDefine, RpRender, RpRenderCanvas, RpRenderPreview, ImageEn, ExtImage, StrUtils;
...
Type TFake=class(TCustomGrid);
     ...
     TPrParams=Record               // Параметры программы
                ...
                IsOwnerPassw:Boolean; // False eckb введен пароль 05235...
...
//       procedure PrintBlankForRegMBCard(CR:TCardInfo);
     ...
     public
       ...
       function  MessBox(Mes:String; Btn:Integer; F:TFont; AutoClose:Integer; var Res:String; URes:Boolean=False; Caption:String=''; bBtn:TWinControl=nil):Integer; overload;
...
//   DM.Qr.SQL.SaveToFile('C:\LOG\RegPr.txt');
  ...
  try
   DM.Qr.Close;
   DM.Qr.SQL.Text:='exec spY_CancelChekRes '+IntToStr(Prm.UserID);
   DM.Qr.ExecSQL;
  except
  end;
...
//  CheckBox2.Visible:=Design;
  ...
  lbExtemp.Caption:='Заказ препаратов по индивидуальным рецептам в аптеке'#10'№1 "Перша",'#10'пл Конституции 20'#10'тел. 050-400-83-62 и '#10'Аптека №308 ул Клочковская 337А'#10'тел. 338-19-92, 773-25-84, 050-400-60-96';

  lbExt.Caption:='Заказ препаратов по индивидуальным рецептам в '#10'Аптеке №1 "Перша",пл Конституции 20 тел. 050-400-83-62 и '#10'Аптека №308 ул Клочковская 337А тел. 338-19-92, 773-25-84, 050-400-60-96';
...
function TMainF.MessBox(Mes:String; Btn:Integer; F:TFont; AutoClose:Integer; var Res:String; URes:Boolean=False; Caption:String='';  bBtn:TwinControl=nil):Integer;
 begin
...
var NC:Int64;
 ...
 var IsSkd:Boolean;
  ...
  begin
   ...
   if (Prm.FirmID=13) and (Prm.AptekaID<>180) and (Prm.AptekaID<>330) then Exit;
...
//  SumChekAll:=DM.spY_CalcSKD.FieldByName('SmAll').AsFloat;
  ...
  FMaxSpisSum:=DM.spY_CalcSKD.FieldByName('MaxSpisSum').AsCurrency;
  DM.spY_CalcSKD.Close;
...
//   if Not (((FEnterKol=True) and (FSkdCalced=False)) or (FEnterKol=False)) then Exit;
 ...
 Begin
  try
   ...
   CalcSkd;
   if ((Prm.FirmID<>13) or ((Prm.FirmID=13) and ( (Prm.AptekaID=180) or (Prm.AptekaID=330) ))) and
...
var tmp,Ostat,CKol,{AC,} PkolAdd,KolD,i,Upak,Koef,Koef1,Kol,KolAdd,ArtAdd,Art_Code,Art_CodeSafe,Old_Art,F_NDS,Type_Map:Integer;
    Srok,T:TDateTime;
  ...
  begin
{
 ...
 //  dbgSklad.Enabled:=False;
   ...
   if DM.spY_FilterSklad.FieldByName('art_code').AsInteger in [1,2] then
    begin
...

{
...

 {
      ...
      begin
       if (FIsSkd7=False) and (Prm.AptekaSklad) and (Prm.Dobraia=False) and (RoundTo(DM.spY_FilterSklad.FieldByName('OstatUp').AsFloat,-2)>=3) and
        ...
        begin
         ...
         MainF.MessBox('"У нас при покупке от 3 упаковок одного товара или 3-x разных наименований в чеке действует оптовая цена, что очень выгодно.'#10+
   ...
   if (Prm.AptekaID<>240) and (Prm.FirmID<>14) and (Prm.FirmID<>15) and (Prm.FirmID<>16) then
 ...
 begin
  ...
  if ChekOpened then
   begin
    ...
    if DM.qrChek.FieldByName('art_code').AsInteger in [1,2] then
     begin
...
procedure TMainF.sbCardClick(Sender:TObject);
 begin
  ...
  if ((Prm.FirmID=13) and (Prm.AptekaID<>180) and (Prm.AptekaID<>330)) {or (Prm.IsSoc and (FTypeChek=TC_STRETCH) ) } then begin TSpeedButton(Sender).Down:=False; Exit; end;
...
var CP:TChekPos;
 ...
 var i:Integer;
  begin
   ...
   if CP.IDRES<>-1 then CP.IsPayMaster:=FIsPayMaster;
   CP.DopTypeOplat:=P;
...
procedure TMainF.CloseChekExecute(Sender:TObject);
 begin
  if TControl(Sender).Enabled then
   begin
    if (FIDRES<>-1) and (FIsPayMaster=True) then PayChek(TO_CHEK,DO_BEZGOT1) else
...
//   TFake(db).RowHeights[2]:=60;
  ...
  if (Prm.AptekaSklad=False) and (DataCol in [5]) and (db.DataSource.DataSet.FieldByName('IsSpisAll').AsInteger=1) then
...
//      DM.ADOCo.BeginTrans;
      try
       ...
       DM.spY_AddToReserve.Parameters.ParamValues['@TypeZakaz']:=1;
        ...
        if Fid_doctor>0 then
         DM.spY_AddToReserve.Parameters.ParamValues['@Priznak']:=4 else
   ...
   for i:=1 to DM.Qr6.RecordCount do
    begin
     ...
     SndRecXML.XMLBody:='<?xml version="1.0" encoding="UTF-8"?>'+#10+
                        ...
                        ' <card_code>'+GetNumMediCard(StrToInt64(DM.Qr6.FieldByName('cardpref').AsString))+'</card_code>'+#10+
     ...
     begin
      ...
      if Copy(IntToStr(NumCardMag),1,3)='252' then // флаер на Сингуляр
       try
        ...
        DM.QrX.SQL.Add('exec spY_GetKolSingular '+IntToStr(Prm.UserID)+','+Copy(IntToStr(NumCardMag),1,12)+',@art output, @Kol1 output, @Kol2 output, @SError output, @IError output');
...
procedure TMainF.miForProvisClick(Sender:TObject);
 begin
  ...
  if DM.QrEx.IsEmpty=False then
   begin
    N76.Visible:=DM.QrEx.FieldByName('value').AsInteger=1;
    N80.Visible:=DM.QrEx.FieldByName('value').AsInteger=1;
    N88.Visible:=DM.QrEx.FieldByName('value').AsInteger=1;
   end; 
...
//  ShowMessage(DM.QrEx.FieldByName('Value').AsString);
  Res:=ShellExecute(0,'open',PChar(DM.QrEx.FieldByName('Value').AsString),nil,nil,SW_SHOWNORMAL);
  if Res<32 then
   WinExec(PChar(DM.QrEx.FieldByName('Value').AsString),SW_SHOW);
 end;
...
var Sum:Currency;
 ...
 begin
   ...
   DM.QrX2.Close;
   DM.QrX2.SQL.Text:='exec [192.168.0.9\sql5].workwith_gamma.dbo.spY_GetSumForZakazObsl '+IntToStr(idres);
