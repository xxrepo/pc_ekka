...
uses
  ...
  Util, ShowTextU, ADODB, Dialogs, Math;
//  , Messages, ShowTextU, Variants;
...
uses
  ...
  VozrPartU, ReturnToProviderU;

{$R *.dfm}

procedure TViewNaklF.ReturnList;
begin
  if (assigned(ViewNaklF))and(ViewNaklF.Showing) then
    ...
    begin
      ...
      begin
        ...
        SQL.Add('select ');//  vz.descr as vz_descr
        SQL.Add('  j.iddoc, j.nn_nakl, j.date_nakl, r.kod_name, s.art_code, j.f_nds, convert(uniqueidentifier,j.id_postav) as id_postav, j.nn_prih, j.dt_prih,');
        SQL.Add('  apteka_net.dbo.GetNamesUpak(p.names) as Names, p.EdName, p.koef, p.manufacturer,');
        SQL.Add('  r.kol, r.cena, r.pr_vz_post,');
        SQL.Add('  post.descr, post.adres,');
        SQL.Add('  i.numseriya, i.SrokGodn,');
        SQL.Add('  vz.descr as vz_descr,');
        SQL.Add('  j.VzDescr');
        SQL.Add('from');
        ...
        SQL.Add('  left join serii i with (nolock) on convert(binary(16),i.id_part8)=convert(binary(16),s.id_part8)');
...
begin
  ...
  begin
    SQL.Clear;
    SQL.Add('declare @kod_name int set @kod_name = convert(int,:kod_name)');
    SQL.Add('declare @art_code int set @art_code = convert(int,:art_code)');
    SQL.Add('select');
    SQL.Add('  j.iddoc, r.kol, r.cena');
    SQL.Add('from');
    SQL.Add('  jmoves j with (nolock) ');
    ...
    SQL.Add('where');
    SQL.Add('  convert(int,j.type_nakl) in (1) and convert(int,r.kod_name) = convert(int,@kod_name) and convert(int,s.art_code) = convert(int,@art_code)');
    Parameters.Clear;
    Parameters.CreateParameter('kod_name',ftInteger,pdInput,10,ADOQuery1.FieldByName('kod_name').Value);
...
begin
  if (DM.spY_NaklList.Active) and (DM.spY_NaklList.FieldByName('Date_Nakl').Value < Date() - 3) then exit;
//  if DM.srNaklList.FieldByName('Date_Nakl').Value
  ...
  try
    ...
    begin
      ...
      begin
        ...
        SQL.Add('declare @iddoc int set @iddoc=convert(int,:iddoc)');
        SQL.Add('select * from Jmoves (nolock) ');
        SQL.Add('where type_nakl = 1 and reoc=0 and obl in (0, 1) and date_nakl >= @dt1 and date_nakl <= @dt2 and not id_postav is null ');
        SQL.Add('  and iddoc = @iddoc');
        SQL.Add('order by date_nakl, nn_nakl');
        Parameters.Clear;
        Parameters.CreateParameter('dt1',ftDateTime,pdInput,23,FormatDateTime('YYYY-MM-DD',dtStart.Date));
        Parameters.CreateParameter('dt2',ftDateTime,pdInput,23,FormatDateTime('YYYY-MM-DD',dtEnd.Date));
        Parameters.CreateParameter('iddoc',ftInteger,pdInput,10,DM.spY_NaklList.FieldByName('iddoc').Value);
        Open;
      end;
    end;
...
begin
  try
    ...
    ReturnList;
    ADOQuery1.First;
    code_name:=ADOQuery1.FieldByName('kod_name').Value;
    with ADOQuery2 do
    begin
      SQL.Clear;
      ...
      SQL.Add('where');
      SQL.Add('  convert(int,r.kod_name) = convert(int,@kod_name)');
      SQL.Add('  and convert(int,r.iddoc)=convert(int,j.iddoc)');
      SQL.Add('  and j.type_nakl=1');
      Parameters.Clear;
      Parameters.CreateParameter('kod_name',ftInteger,pdInput,10,code_name);
      Open;
    end;
    ...
    begin
      ...
      if not ADOQuery1.FieldByName('dt_prih').IsNull then
        PrintRep.AddText(' '+FormatDateTime('DD MMM YYYY',ADOQuery1.FieldByName('dt_prih').Value)+' '+#10+#10)
      else
...
//      Tb.Cell[3,i].Align:=AL_CENTER; Tb.Cell[3,i].AddText(ADOQuery1.FieldByName('EdName').Value);
      ...
      kol_naim:=kol_naim+1;{ADOQuery1.FieldByName('kol').Value;}
...
//    cols[2].flds:=[21, 30];
    ...
    PrintRep.AddText(' от ');
    PrintRep.Font.Style:=[fsUnderline,fsBold];
    if not ADOQuery1.FieldByName('dt_prih').IsNull then PrintRep.AddText(' '+FormatDateTime('DD MMMM YYYY',ADOQuery1.FieldByName('dt_prih').Value)+' ') else PrintRep.AddText(white_spaces);
    PrintRep.Font.Style:=[];
...
//      if (ADOQuery2.FieldByName('pr_vz_post').Value=9)or(ADOQuery2.FieldByName('pr_vz_post').Value=10)or(ADOQuery2.FieldByName('pr_vz_post').Value=26) then //недовоз
...
//      if (ADOQuery2.FieldByName('pr_vz_post').Value=21)or(ADOQuery2.FieldByName('pr_vz_post').Value=30) then //товар не заказан
    ...
    while not ADOQuery2.Eof do
    begin     //'Недовоз','Брак','Товар не заказан','Инструкции','Фарминспекция','Серия','Срок годности'
//      if (ADOQuery2.FieldByName('pr_vz_post').Value=9)or(ADOQuery2.FieldByName('pr_vz_post').Value=10)or(ADOQuery2.FieldByName('pr_vz_post').Value=26) then cols[0].vis_f:=true; //недовоз
...
//      if (ADOQuery2.FieldByName('pr_vz_post').Value=21)or(ADOQuery2.FieldByName('pr_vz_post').Value=30) then cols[2].vis_f:=true; //товар не заказан
...
//        if ADOQuery1.FieldByName('pr_vz_post').AsInteger in [9,10,26] then
        if ADOQuery1.FieldByName('pr_vz_post').AsInteger in [26] then
        begin
//          if cols[i].flds=[9,10,26] then //Недовоз
...
//            if ADOQuery1.FieldByName('pr_vz_post').AsInteger in [21,30] then //Товар не заказан
            if ADOQuery1.FieldByName('pr_vz_post').AsInteger in [30] then //Товар не заказан
            begin
//              if cols[i].flds=[21,30] then
...
//'Недовоз','Брак','Товар не заказан','Инструкции','Фарминспекция','Серия','Срок годности'
    ...
    begin
      ...
      Font.Style:=[]; AddText(' от '); Font.Style:=[fsBold,fsUnderline]; if not ADOQuery1.FieldByName('dt_prih').IsNull then PrintRep.AddText(' '+FormatDateTime('DD MMMM YYYY',ADOQuery1.FieldByName('dt_prih').Value)+' ') else PrintRep.AddText(white_spaces);
...
begin
  try
    ...
    Tb.Cell[1,1].AddText('Возврат поставщику '); if not ADOQuery1.FieldByName('descr').IsNull then Tb.Cell[1,1].AddText(' '+ADOQuery1.FieldByName('descr').Value+' '+#10) else Tb.Cell[1,1].AddText(' '+underlines+' '+#10);
...
begin
  try
    ...
    Tb.Cell[2,1].AddText('№ '+underlines+#10+#10);
    if not ADOQuery1.FieldByName('descr').IsNull then Tb.Cell[2,1].AddText(' '+ADOQuery1.FieldByName('descr').Value+' '+#10) else Tb.Cell[2,1].AddText(' '+underlines+underlines+' '+#10);
    ...
    PrintRep.Font.Style:=[fsBold,fsUnderline];
    if not ADOQuery1.FieldByName('dt_prih').IsNull then PrintRep.AddText(FormatDateTime('DD MMMM YYYY',ADOQuery1.FieldByName('dt_prih').Value)+#10) else PrintRep.AddText(underlines+#10);
    PrintRep.AddText(#10);
...
procedure TViewNaklF.BitBtn16Click(Sender: TObject);
type
  TReturn = set of Byte;
  ttmp = record
    vis_f: boolean;
...
const
  white_spaces='               ';
  underlines = '_____________________';
  header: array[0..13] of string = ('№ п/п','Арткод','Наименование товара','Серия','Цена (грн.)','Количество по накладной','Фактическое количество принятого товара','Недовоз','Брак','Товар не заказан','Инструкции','Фарминспекция','Серия','Срок годности');
  garanie = 'Гарантирую, что все лекарственные средства, указанные в этой накладной, на протяжении всего периода нахождения в аптеке хранились в соответствии с условиями хранения, утвержденными производителем.';
...
//    cols[2].flds:=[21, 30];
    ...
    begin  //'Недовоз','Брак','Товар не заказан','Инструкции','Фарминспекция','Серия','Срок годности'
      //недовоз
//      if (ADOQuery2.FieldByName('pr_vz_post').Value=9)or(ADOQuery2.FieldByName('pr_vz_post').Value=10)or(ADOQuery2.FieldByName('pr_vz_post').Value=26) then
      ...
      //брак
      if (ADOQuery2.FieldByName('pr_vz_post').Value=3) then row_count:=row_count+1;
      //срок годности
      if (ADOQuery2.FieldByName('pr_vz_post').Value=4) then row_count:=row_count+1;
      //товар не заказан
//      if (ADOQuery2.FieldByName('pr_vz_post').Value=21)or(ADOQuery2.FieldByName('pr_vz_post').Value=30) then
    ...
    case row_count of
       7: Tb.SetWidths('250,250,1000,250,250,250,250');
       8: Tb.SetWidths('250,250,1000,250,250,250,250,250');
       9: Tb.SetWidths('250,250,1000,250,250,250,250,250,250');
      10: Tb.SetWidths('250,250,1000,250,250,250,250,250,250,250');
      11: Tb.SetWidths('250,250,1000,250,250,250,250,250,250,250,250');
      12: Tb.SetWidths('250,250,1000,250,250,250,250,250,250,250,250,250');
      13: Tb.SetWidths('250,250,1000,250,250,250,250,250,250,250,250,250,250');
      14: Tb.SetWidths('250,250,1000,250,250,250,250,250,250,250,250,250,250,250');
    end;
    for i:=0 to 6 do
    begin
      Tb.Cell[i+1,1].AddText(header[i]+#10);
...
//      if (ADOQuery2.FieldByName('pr_vz_post').Value=21)or(ADOQuery2.FieldByName('pr_vz_post').Value=30) then cols[2].vis_f:=true; //товар не заказан
    ...
    j:=8;
    i:=0;
    repeat //'Недовоз','Брак','Товар не заказан','Инструкции','Фарминспекция','Серия','Срок годности'
      if cols[i].vis_f then
      begin
        cols[i].col:=j;
        Tb.Cell[j,1].AddText(header[j]);
    ...
    begin
      //№ п/п
      Tb.Cell[1,row_no].Align:=AL_CENTER;
      Tb.Cell[1,row_no].AddText(IntToStr(row_no-1));
      //арткод
      Tb.Cell[2,row_no].Align:=AL_CENTER;
      Tb.Cell[2,row_no].AddText(ADOQuery1.FieldByName('art_code').Value);
      //Наименование товара
      Tb.Cell[3,row_no].Align:=AL_CENTER;
      Tb.Cell[3,row_no].AddText(ADOQuery1.FieldByName('names').Value);
      //Серия
      Tb.Cell[4,row_no].Align:=AL_CENTER;
      if not ADOQuery1.FieldByName('numseriya').IsNull then Tb.Cell[4,row_no].AddText(ADOQuery1.FieldByName('numseriya').Value);
      //Цена (грн.)
      Tb.Cell[5,row_no].Align:=AL_CENTER;
      Tb.Cell[5,row_no].AddText( FloatToStrF( RoundCurr(ADOQuery1.FieldByName('cena').Value / ADOQuery1.FieldByName('koef').Value),10,2 ));

      //Количество по накладной
      IncomingNakl;
      Tb.Cell[6,row_no].Align:=AL_CENTER;
      Tb.Cell[6,row_no].AddText( FloatToStrF( ADOQuery2.FieldByName('kol').Value / ADOQuery1.FieldByName('koef').Value,10,3 ));

      //Фактическое количество принятого товара
      Tb.Cell[7,row_no].Align:=AL_CENTER;
      Tb.Cell[7,row_no].AddText(FloatToStrF((ADOQuery2.FieldByName('kol').Value-ADOQuery1.FieldByName('kol').Value)/ADOQuery1.FieldByName('koef').Value,10,3));

      //Недовоз или Брак или Товар не заказан или Инструкции или Фарминспекция или Серия или Срок годности
      kol_return:=FloatToStrF(ADOQuery1.FieldByName('kol').Value/ADOQuery1.FieldByName('koef').Value,10,3);
      for i:=8 to row_count do begin Tb.Cell[i,row_no].Align:=AL_CENTER;end;
...
//      //Срок годности
    ...
    Tb.Cell[1,rec_count-1].AddText('НДС, грн.');
    Tb.Cell[1,rec_count].AddText('Всего с НДС, грн.');
    Tb.SetBorders(8,rec_count-4,row_count,rec_count,DEFAULT_BORDER);
...
//'Недовоз','Брак','Товар не заказан','Инструкции','Фарминспекция','Серия','Срок годности'
    ...
    repeat
      if cols[i].vis_f then
      begin
        Tb.Cell[j,rec_count-3].AddText(FloatToStrF(kol[i],10,3)); //Всего, шт.
        Tb.Cell[j,rec_count-2].AddText(FloatToStrF(RoundCurr(sum[i]),10,2)); //Всего, грн.
        Tb.Cell[j,rec_count-1].AddText(FloatToStrF(RoundCurr(sum_nds[i]),10,2)); //НДС, грн.
        Tb.Cell[j,rec_count].AddText(FloatToStrF(RoundCurr(sum[i]+sum_nds[i]),10,2)); //Всего с НДС, грн.
        inc(j);
      end;
      sum_all:=sum_all+sum[i]+sum_nds[i];
      kol_all:=kol_all+kol[i];
    ...
    begin
      Font.Style:=[fsBold];
      AddText(#10+'Итого, при проверке фактически поставленного товара, выявлено несоответствие данных, указаных в накладной, всего в количестве ');
      Font.Style:=[fsBold,fsUnderline]; AddText(FloatToStrF(kol_all,10,3)+' ('+FloatToWordsRU(kol_all,10,3)+') ');
      Font.Style:=[fsBold]; AddText(' шт. на общую сумму ');
      Font.Style:=[fsBold,fsUnderline]; AddText(CurrToWordsRU(sum_all,0));
      Font.Style:=[]; AddText(#10+#10+'Несоответствие: '+#10+#10);
      AddText('В результате выявленых несоответствий по накладной № ');
