...
type
  ...
  public
    ...
    procedure PrepareUserList(P:Byte=0); // Загрузка списка пользователей
 ...
 begin
  try
   ...
   // if OpenADOSQL(DM.Qr,'select *,IsNull(DtEnd,getdate()+10) as DtEnds from SPRUSER (nolock) where reserve=0 and Users='''+ComboBox1.Text+'''')<=0 then raise EAbort.Create('');
    ...
    if TrimRight(MaskEdit1.Text)<>TrimRight(DM.Qr.FieldByName('Passw').AsString) then raise EAbort.Create('Пароль набран неверно!');
...
//           DM.Qr.SQL.Text:='select id_state from inform..e_daily_journal where id_daily_operation=1 and daily_operation_time>convert(datetime,convert(varchar,getdate(),23)) and id_employee='+IntToStr(Prm.ID_Gamma);
           ...
           DM.Qr.Open;
           if (DM.Qr.IsEmpty) then Mes:='Вы не зарегистрировали приход на работу через сканер пальцев!'#10+'Если у вас действительно не сканируются пальцы звоните в офис оператору видеонаблюдения и вам сообщат единоразовый пароль для входа в программу!';
          end;


{
         if DM.Qr.FieldByName('id_state').AsInteger=0 then Mes:='Вы зарегистрировали свой приход на работу как подтверждение через палец заведующей! Необходимо зарегистрировать приход на работу через свой палец!'#10+
                                                                'Если у вас действительно не сканируются пальцы звоните в офис оператору видеонаблюдения и вам сообщат единоразовый пароль для входа в программу!';
}
         if Mes<>'' then
          begin
