      ...
      begin
{
        ...
        SQL.Add('declare @iddoc int set @iddoc = :iddoc');
        SQL.Add('select ');
        SQL.Add('  j.iddoc, j.nn_nakl, j.date_nakl, s.art_code, j.f_nds, convert(uniqueidentifier,j.id_postav) as id_postav, j.nn_prih, j.dt_prih,');
        SQL.Add('  apteka_net.dbo.GetNamesUpak(p.names) as Names, p.koef, p.manufacturer,');
        SQL.Add('  sum(r.kol) as kol, max(r.cena) as cena, r.pr_vz_post,');
        SQL.Add('  post.descr, post.adres,');
        SQL.Add('  i.numseriya, i.SrokGodn,');
        SQL.Add('  vz.descr as vz_descr,');
        SQL.Add('  j.VzDescr');
        ...
        SQL.Add('  j.iddoc = @iddoc');
        SQL.Add('group by');
        SQL.Add('  j.iddoc, j.nn_nakl, j.date_nakl, s.art_code, j.f_nds, j.id_postav, j.nn_prih, j.dt_prih,');
        SQL.Add('  p.names, p.koef, p.manufacturer, r.pr_vz_post, post.descr, post.adres, i.numseriya, i.SrokGodn, vz.descr, j.VzDescr');
        SQL.Add('order by');
        SQL.Add('  j.iddoc, j.nn_nakl');
        Parameters.Clear;
        Parameters.CreateParameter('iddoc',ftInteger,pdInput,10,DM.spY_NaklList.FieldByName('iddoc').Value);
        Open;
{
        ...
        SQL.Add('declare @iddoc int set @iddoc = :iddoc');
        SQL.Add('select');
        SQL.Add('  j.iddoc, j.F_NDS, j.Type_Tov, j.ID_USER, j.SKLAD, j.nn_nakl, j.date_nakl, j.nn_prih, j.dt_prih, j.VzDescr, j.type_nakl, j.reoc, j.obl, convert(uniqueidentifier,j.id_postav) as id_postav,');
        SQL.Add('  r.pr_vz_post, r.SHTRIH, r.ISSCAN, sum(r.kol) as kol, max(r.cena) as cena, max(r.CENAP) as cenap, r.kod_name, ');
        SQL.Add('  s.NAMES, s.ART_CODE, s.ART_NAME,');
        SQL.Add('  p.koef, p.manufacturer, apteka_net.dbo.GetNamesUpak(p.names) as Names,');
        SQL.Add('  i.numseriya, i.srokgodn,');
        SQL.Add('  post.descr, post.adres,');
        SQL.Add('  vz.descr as vz_descr');
        SQL.Add('from');
        ...
        SQL.Add('group by');
        SQL.Add('  j.iddoc, j.F_NDS, j.Type_Tov, j.ID_USER, j.SKLAD, j.nn_nakl, j.date_nakl, j.nn_prih, j.dt_prih, j.VzDescr, j.type_nakl, j.reoc, j.obl, j.id_postav,');
        SQL.Add('  r.pr_vz_post, r.SHTRIH, r.ISSCAN, r.kod_name,');
        SQL.Add('  s.NAMES, s.ART_CODE, s.ART_NAME,');
        SQL.Add('  p.koef, p.manufacturer, p.names,');
        SQL.Add('  i.numseriya, i.srokgodn,');
        SQL.Add('  post.descr, post.adres,');
        SQL.Add('  vz.descr');
        SQL.Add('order by');
        SQL.Add('  j.iddoc, j.nn_nakl, s.names, s.ART_CODE, r.kod_name');
        Parameters.Clear;
        Parameters.CreateParameter('iddoc',ftInteger,pdInput,10,DM.spY_NaklList.FieldByName('iddoc').Value);
        Open;
}        
...
begin
  ...
  begin
    ...
    SQL.Add('select');
    SQL.Add('  j.type_nakl, s.art_code, j.F_NDS, j.id_postav, j.nn_prih, j.dt_prih, j.iddoc, sum(r.kol) as kol, max(r.cena) as cena');
    SQL.Add('from');
    SQL.Add('  jmoves j with (nolock)');
    ...
    SQL.Add('group by');
    SQL.Add('  j.type_nakl, s.art_code, j.F_NDS, j.id_postav, j.nn_prih, j.dt_prih, j.iddoc');
    Parameters.Clear;
    ...
    Parameters.CreateParameter('art_code',ftInteger,pdInput,10,ADOQuery1.FieldByName('art_code').Value);
{
    ...
    SQL.Add('declare @art_code int set @art_code = convert(int,:art_code)');
    SQL.Add('select');
    SQL.Add('  j.iddoc, r.kod_name, r.kol, r.cena');
    SQL.Add('from');
    SQL.Add('  jmoves j with (nolock) ');
    SQL.Add('  left join rtovar r with (nolock) on convert(int,r.iddoc) = convert(int,j.iddoc)');
    SQL.Add('  inner join sprtov s with (nolock) on convert(int,s.kod_name) = convert(int,r.kod_name)');
    ...
    begin
     ...
     for iii:=1 to 2 do
      begin
       ...
       DM.Qr.Close;
       DM.Qr.SQL.Clear;
       DM.Qr.SQL.Add('select nn_nakl+''-'+IntToStr(Prm.AptekaID)+''' +case Priznak when 19 then ''     ЯЩИКИ'' when 20 then ''     КРЫШКИ'' else '''' end as [Номера накладных_S_600], ');
...
begin
  ...
  try
    ...
    if not ViewNaklF.ADOQuery1.FieldByName('iddoc').IsNull then ReturnToProviderF.lblIDDoc.Caption:=ViewNaklF.ADOQuery1.FieldByName('iddoc').Value else ReturnToProviderF.lblIDDoc.Caption:='';
...
begin
  try
    ...
    ADOQuery1.First;
//    code_name:=ADOQuery1.FieldByName('kod_name').AsInteger;
//    with ADOQuery2 do
//    begin
//      SQL.Clear;
...
//      if (ADOQuery2.FieldByName('pr_vz_post').Value=21)or(ADOQuery2.FieldByName('pr_vz_post').Value=30) then cols[2].vis_f:=true; //товар не заказан
    ...
    j:=0;
    ADOQuery1.First;
    iddoc:=ADOQuery1.FieldByName('iddoc').Value;
    while not ADOQuery1.Eof do
    begin
...
//    if not ADOQuery1.FieldByName('descr').IsNull then Tb.Cell[1,1].AddText(' '+ADOQuery1.FieldByName('descr').Value+' '+#10) else Tb.Cell[1,1].AddText(' '+underlines+' '+#10);
