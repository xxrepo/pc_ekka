...
//       FSumFransh:Currency;
       ...
       procedure PrevEnterKol(Param:Integer; IsFirstRec:Integer);
...
var Reg:TRegistry;
 ...
 begin
  ...
  if (FIP='10.44.4.17') or (FIP='10.31.70.10') or
  ...
  try
   ...
   try
    ...
    Reg.WriteString('ProxyServer','192.168.0.240:3128');
...

{
  ...
  if (Prm.AptekaID=230) and (Opt.KassaID=21) then EKKA.fpCutBeep(0,0,0)
                                             else EKKA.fpCutBeep(1,0,0);
 ...
 Begin
  try
   ...
   if ((Prm.FirmID<>13) or ((Prm.FirmID=13) and ( (Prm.AptekaID=180) or (Prm.AptekaID=330) or (Prm.AptekaID=462) ))) and
      ...
      ((FIDRes=-1) or ((FIDRes<>-1) and (FTypeRes=2)) ) then
  ...
  begin
   ...
   if db.DataSource.DataSet.Locate('ART_CODE',AnsiUpperCase(IntToStr(AC)),[loCaseInsensitive,loPartialKey]) then
    begin
     if B then
      begin
       PrevEnterKol(EK_SCAN,0);

       try // Регистрируем все, что сканируется
        DM.QrEx5.Close;
        DM.QrEx5.SQL.Text:='exec spY_RegScan '''+EAN13+''','+IntToStr(Prm.ID_Gamma)+','+IntToStr(Opt.KassaID);
        DM.QrEx5.ExecSQL;
       except
       end;
...
//    if hKonkurHintF1.Visible then hKonkurHintF1.Hide;
  ...
  if Key=VK_RETURN then PrevEnterKol(EK_DIALOG,0);
  FIsKeyDown:=True;
...
procedure TMainF.PrevEnterKol(Param:Integer; IsFirstRec:Integer);
  ...
  begin
{
 ...
 //  dbgSklad.Enabled:=False;
   ...
   if DM.spY_FilterSklad.FieldByName('art_code').AsInteger in [1,2,7] then
    begin
...

{
   if (TypeChek=TC_PILOT) then
    begin
     DM.QrEx.Close;
     DM.QrEx.SQL.Text:='exec spY_CntNamesChek '+IntToStr(Prm.UserID)+','+DM.spY_FilterSklad.FieldByName('art_code').AsString;
     DM.QrEx.Open;
     if DM.QrEx.FieldByName('cnt').AsInteger>=1 then
      begin
       MessBox('По рецепту можно добавлять только одно наименование в чек!');
       Exit;
      end;
    end;
}
...
{
   if (Fid_strah>0) and (DM.spY_FilterSklad.FieldByName('F_NDS').AsInteger=1) then
    begin
     if (AnsiUpperCase(Copy(DM.spY_FilterSklad.FieldByName('names').AsString,1,5))='ПАКЕТ') then
      begin
       MessBox('Данный чек по страховой компании! Пакеты необходимо пробивать отдельным чеком!');
       Exit;
      end;

     if MessBox('Данная позиция имеет ставку НДС 20%. Вы согласовали продажу данной позиции со страховой?',52)<>ID_YES then Exit;
    end;
}
...

 {
   ...
   (* -- Закоментировано блокировку на пробитие просроченных товаров 09.08.17 так как надоело туда сюда открывать
     ...
     if pnBarDop.Tabs.Items[2].Btn.Tag>0 then
      try
       ...
       try
        ...
        PrepareIm(imHist,pnBarDop.Tabs.Items[2].Btn.Tag,Bm);
...
var db:TDBGrid;
    dR,sR:TRect;
    dx,dy,dxr,Kl,Art_Code,i,Ostat,dx2,dy2:Integer;
    PCena:String;
    tmpBColor,C:TColor;
    Srok:TDateTime;
    IsS,IsRec:Boolean;
   ...
   if Tg=2 then
    ...
    begin
     ...
     try
      ...
      PrOtkazF.rb4.Visible:=Prm.AptekaSklad or IsLgot;
...

{
       ...
       for i:=1 to DM.qrResPos.RecordCount do
        begin
         ...
         if FId_strah>0 then DM.spY_EnterKol.Parameters.ParamValues['@simp']:=1
                        else DM.spY_EnterKol.Parameters.ParamValues['@simp']:=0;
      ...
      if dbgSklad.DataSource.DataSet.Locate('Art_Code',FirstRecomendF.Flag,[loCaseInsensitive,loPartialKey]) then
       begin
        if dbgSklad.DataSource.DataSet.FieldByName('Ostat').AsInteger<=0 then B:=False
                                                                         else PrevEnterKol(EK_DIALOG,FirstRecomendF.ArtDop);
       end else B:=False;
...
var  A,B,i,j,Rc,Cc:Integer;
 ...
 begin
  ...
  ShowMessage(FormatDateTime('hh:nn:ss.zzzzzz',Time-TT));
...
procedure TMainF.dbgUpDblClick(Sender: TObject);
 begin
  ...
  if dbgSklad.DataSource.DataSet.Locate('Art_Code',TDBGrid(Sender).DataSource.DataSet.FieldByName('Art_Code').AsInteger,[loCaseInsensitive,loPartialKey]) then PrevEnterKol(EK_DIALOG,0);
 end;
...
var Sum:Currency;
 ...
 begin
   ...
   DM.QrX2.Close;
   DM.QrX2.SQL.Text:='exec [911-SQL2\SQL2].workwith_gamma.dbo.spY_GetSumForZakazObsl '+IntToStr(idres);
