 ...
 begin
  ...
  for i:=1 to DM.qrSpis.RecordCount do
   begin
    ...
    aList[i-1].lbKol.Caption:='Количество';
    ...
    aList[i-1].lbBonus.Caption:='Бонус: '+DM.qrSpis.FieldByName('Prize').AsString+' грн.';
    ...
    aList[i-1].lb1.Caption:='Добавить в чек';
      ...
      begin
       ...
       if DM.qrSpis.IsEmpty=False then Param:=3 else
        begin
         ...
         DM.qrSpis.Open;
         if DM.qrSpis.IsEmpty=False then Param:=5 else
          begin
           DM.qrSpis.Close;
           DM.qrSpis.SQL.Clear;
           DM.qrSpis.SQL.Text:='exec spY_ConComit '+IntToStr(lbArt.Tag)+',null,null ';
           DM.qrSpis.Open;
           if DM.qrSpis.IsEmpty=False then Param:=4;
          end
        end;
      end
    end;

   // Правая дополнительная панель "План продаж на аптеку"
...
//     Panel3.Top:=Label54.Top+Label54.Height+10;
   ...
   if Param=2 then // Планы продаж на сотрудников
    begin
...
//     DM.QrEx.SQL.Text:='exec spY_GetSumBonus '+IntToStr(Prm.ID_Gamma)+','+IntToStr(id_prior)+','+IntToStr(id_akcii)+', '''+FormatDateTime('yyyy-mm-dd',StartOfTheMonth(Date-300))+''','''+FormatDateTime('yyyy-mm-dd hh:nn:ss',EndOfTheMonth(Date+300))+'''';
     ...
     if DM.qrSpis.FieldByName('AfterOver').AsCurrency>0 then
      begin
       ...
       Label92.Caption:='При перевыполнении плана (более чем на '+DM.qrSpis.FieldByName('Prc_').AsString+'%)  за каждую упаковку будет';
   ...
   if Param=5 then // Планы продаж на сотрудников групповой
    begin
     id_akcii:=DM.qrSpis.FieldByName('id_plan').AsInteger;
     id_prior:=8;
   ...
   if Param=3 then // Планы продаж на аптеку
    begin
...
//       Label21.Caption:='При перевыполнении плана (более чем на '+DM.qrSpis.FieldByName('Prc_').AsString+'%)  за каждую упаковку будет';
   ...
   if Param=4 then // Комплексная покупка
    begin
     id_prior:=6;
...
var i:Integer;
 begin
  ...
  if EP.cArtConc=0 then
   begin
    MainF.MessBox('Ничего не выбрано из сопутствующих товаров, либо введено нулевое количество');
    Exit;
   end;
