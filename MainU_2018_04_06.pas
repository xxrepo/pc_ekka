...
//procedure TMainF.GetGeptralChek(Sender: TObject; const request_project_id: byte; const request_operation: byte);
var
  Req: OleVariant;
  ...
  begin
    try
      try
        ...
        SQL.Clear;
        //SQL.Text:='exec spY_GetAbbotOrOrangeInChek @GeptralOrOrange = ''гептрал''';
        ...
        begin
          TSpeedButton(Sender).Down:=False;
          raise Exception.Create('В чек может быть добавлено только одно наименование препарата "Гептрал"!');
        end;
        First;
      ...
      begin
        try
          Connection:=DM.ADOCo;
          SQL.Clear;
          SQL.Add('declare @request_time datetime set @request_time = '''+DateTimeToStr(request_time)+'''');
          ...
          SQL.Add('declare @response_time datetime set @response_time = '''+DateTimeToStr(response_time)+'''');
